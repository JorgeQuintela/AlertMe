export const fetchLoopTimer = 5000; //5 sec

export const stocksNameIndustry = [
  {symbol: 'AAPL', name: 'Apple Inc.', industry: 'Computer Manufacturing'},
  {
    symbol: 'MSFT',
    name: 'Microsoft Corporation',
    industry: 'Computer Software: Prepackaged Software',
  },
  {
    symbol: 'GOOG',
    name: 'Alphabet Inc. Class C Capital Stock',
    industry: 'Internet and Information Services',
  },
  {
    symbol: 'GOOGL',
    name: 'Alphabet Inc. Class A',
    industry: 'Internet and Information Services',
  },
  {
    symbol: 'AMZN',
    name: 'Amazon.com Inc.',
    industry: 'Catalog/Specialty Distribution',
  },
  {symbol: 'TSLA', name: 'Tesla Inc.', industry: 'Auto Manufacturing'},
  {
    symbol: 'UNH',
    name: 'UnitedHealth Group Incorporated (DE)',
    industry: 'Medical Specialities',
  },
  {
    symbol: 'JNJ',
    name: 'Johnson & Johnson',
    industry: 'Biotechnology: Pharmaceutical Preparations',
  },
  {
    symbol: 'META',
    name: 'Meta Platforms Inc. Class A',
    industry: 'Internet and Information Services',
  },
  {symbol: 'V', name: 'Visa Inc.', industry: 'Business Services'},
  {
    symbol: 'TSM',
    name: 'Taiwan Semiconductor Manufacturing Company Ltd.',
    industry: 'Semiconductors',
  },
  {symbol: 'NVDA', name: 'NVIDIA Corporation', industry: 'Semiconductors'},
  {
    symbol: 'XOM',
    name: 'Exxon Mobil Corporation',
    industry: 'Oil & Gas Production',
  },
  {
    symbol: 'PG',
    name: 'Procter & Gamble Company (The)',
    industry: 'Package Goods/Cosmetics',
  },
  {
    symbol: 'WMT',
    name: 'Walmart Inc.',
    industry: 'Department/Specialty Retail Stores',
  },
  {symbol: 'JPM', name: 'JP Morgan Chase & Co.', industry: 'Major Banks'},
  {
    symbol: 'BABA',
    name: 'Alibaba Group Holding Limited American Depositary Shares each representing eight Ordinary share',
    industry: 'Catalog/Specialty Distribution',
  },
  {
    symbol: 'MA',
    name: 'Mastercard Incorporated',
    industry: 'Business Services',
  },
  {
    symbol: 'LLY',
    name: 'Eli Lilly and Company',
    industry: 'Biotechnology: Pharmaceutical Preparations',
  },
  {
    symbol: 'PFE',
    name: 'Pfizer Inc.',
    industry: 'Biotechnology: Pharmaceutical Preparations',
  },
  {
    symbol: 'HD',
    name: 'Home Depot Inc. (The)',
    industry: 'RETAIL: Building Materials',
  },
  {
    symbol: 'CVX',
    name: 'Chevron Corporation',
    industry: 'Integrated oil Companies',
  },
  {symbol: 'BAC', name: 'Bank of America Corporation', industry: 'Major Banks'},
  {
    symbol: 'KO',
    name: 'Coca-Cola Company (The)',
    industry: 'Beverages (Production/Distribution)',
  },
  {symbol: 'ABBV', name: 'AbbVie Inc.', industry: 'Other Pharmaceuticals'},
  {
    symbol: 'NVO',
    name: 'Novo Nordisk A/S',
    industry: 'Biotechnology: Pharmaceutical Preparations',
  },
  {
    symbol: 'PEP',
    name: 'PepsiCo Inc.',
    industry: 'Beverages (Production/Distribution)',
  },
  {
    symbol: 'MRK',
    name: 'Merck & Company Inc. (new)',
    industry: 'Biotechnology: Pharmaceutical Preparations',
  },
  {
    symbol: 'COST',
    name: 'Costco Wholesale Corporation',
    industry: 'Department/Specialty Retail Stores',
  },
  {
    symbol: 'TMO',
    name: 'Thermo Fisher Scientific Inc',
    industry: 'Biotechnology: Laboratory Analytical Instruments',
  },
  {
    symbol: 'TM',
    name: 'Toyota Motor Corporation',
    industry: 'Auto Manufacturing',
  },
  {
    symbol: 'VZ',
    name: 'Verizon Communications Inc.',
    industry: 'Telecommunications Equipment',
  },
  {
    symbol: 'GME',
    name: 'GameStop Corporation',
    industry: 'Electronics Distribution',
  },
];
export const allStocks = [
  'A',
  'AA',
  'AAC',
  'AACG',
  'AACI',
  'AADI',
  'AAIC',
  'AAL',
  'AAMC',
  'AAME',
  'AAN',
  'AAOI',
  'AAON',
  'AAP',
  'AAPL',
  'AAQC',
  'AAT',
  'AATC',
  'AAU',
  'AAWW',
  'AB',
  'ABB',
  'ABBV',
  'ABC',
  'ABCB',
  'ABCL',
  'ABCM',
  'ABEO',
  'ABEV',
  'ABG',
  'ABGI',
  'ABIO',
  'ABM',
  'ABMD',
  'ABNB',
  'ABOS',
  'ABR',
  'ABSI',
  'ABST',
  'ABT',
  'ABTX',
  'ABUS',
  'ABVC',
  'AC',
  'ACA',
  'ACAB',
  'ACAC',
  'ACAD',
  'ACAH',
  'ACAQ',
  'ACAX',
  'ACB',
  'ACBA',
  'ACC',
  'ACCD',
  'ACCO',
  'ACDI',
  'ACEL',
  'ACER',
  'ACET',
  'ACEV',
  'ACGL',
  'ACH',
  'ACHC',
  'ACHL',
  'ACHR',
  'ACHV',
  'ACI',
  'ACII',
  'ACIU',
  'ACIW',
  'ACKI',
  'ACLS',
  'ACLX',
  'ACM',
  'ACMR',
  'ACN',
  'ACNB',
  'ACON',
  'ACOR',
  'ACQR',
  'ACR',
  'ACRE',
  'ACRO',
  'ACRS',
  'ACRX',
  'ACST',
  'ACT',
  'ACTD',
  'ACTG',
  'ACU',
  'ACVA',
  'ACXP',
  'ADAG',
  'ADAL',
  'ADAP',
  'ADBE',
  'ADC',
  'ADCT',
  'ADER',
  'ADES',
  'ADEX',
  'ADGI',
  'ADI',
  'ADIL',
  'ADM',
  'ADMA',
  'ADMP',
  'ADN',
  'ADNT',
  'ADOC',
  'ADP',
  'ADPT',
  'ADRA',
  'ADRT',
  'ADSE',
  'ADSK',
  'ADT',
  'ADTH',
  'ADTN',
  'ADTX',
  'ADUS',
  'ADV',
  'ADVM',
  'ADXN',
  'ADXS',
  'AE',
  'AEAC',
  'AEAE',
  'AEE',
  'AEG',
  'AEHA',
  'AEHL',
  'AEHR',
  'AEI',
  'AEIS',
  'AEL',
  'AEM',
  'AEMD',
  'AENZ',
  'AEO',
  'AEP',
  'AER',
  'AERC',
  'AERI',
  'AES',
  'AESE',
  'AEVA',
  'AEY',
  'AEYE',
  'AEZS',
  'AFAC',
  'AFAQ',
  'AFAR',
  'AFBI',
  'AFCG',
  'AFG',
  'AFIB',
  'AFL',
  'AFMD',
  'AFRI',
  'AFRM',
  'AFTR',
  'AFYA',
  'AG',
  'AGAC',
  'AGBA',
  'AGCB',
  'AGCO',
  'AGE',
  'AGEN',
  'AGFS',
  'AGFY',
  'AGGR',
  'AGI',
  'AGIL',
  'AGIO',
  'AGL',
  'AGLE',
  'AGM',
  'AGM.',
  'AGMH',
  'AGNC',
  'AGO',
  'AGR',
  'AGRI',
  'AGRO',
  'AGRX',
  'AGS',
  'AGTC',
  'AGTI',
  'AGX',
  'AGYS',
  'AHCO',
  'AHG',
  'AHH',
  'AHI',
  'AHPA',
  'AHPI',
  'AHRN',
  'AHT',
  'AI',
  'AIB',
  'AIG',
  'AIH',
  'AIHS',
  'AIKI',
  'AIM',
  'AIMA',
  'AIMC',
  'AIN',
  'AINC',
  'AINV',
  'AIP',
  'AIR',
  'AIRC',
  'AIRG',
  'AIRI',
  'AIRS',
  'AIRT',
  'AIT',
  'AIU',
  'AIV',
  'AIZ',
  'AJG',
  'AJRD',
  'AJX',
  'AKA',
  'AKAM',
  'AKAN',
  'AKBA',
  'AKIC',
  'AKO.',
  'AKO.',
  'AKR',
  'AKRO',
  'AKTS',
  'AKTX',
  'AKU',
  'AKUS',
  'AKYA',
  'AL',
  'ALB',
  'ALBO',
  'ALC',
  'ALCC',
  'ALCO',
  'ALDX',
  'ALE',
  'ALEC',
  'ALEX',
  'ALF',
  'ALG',
  'ALGM',
  'ALGN',
  'ALGS',
  'ALGT',
  'ALHC',
  'ALIM',
  'ALIT',
  'ALJJ',
  'ALK',
  'ALKS',
  'ALKT',
  'ALL',
  'ALLE',
  'ALLG',
  'ALLK',
  'ALLO',
  'ALLR',
  'ALLT',
  'ALLY',
  'ALNA',
  'ALNY',
  'ALOR',
  'ALOT',
  'ALPA',
  'ALPN',
  'ALPP',
  'ALR',
  'ALRM',
  'ALRN',
  'ALRS',
  'ALSA',
  'ALSN',
  'ALT',
  'ALTG',
  'ALTO',
  'ALTR',
  'ALTU',
  'ALV',
  'ALVO',
  'ALVR',
  'ALX',
  'ALXO',
  'ALYA',
  'ALZN',
  'AM',
  'AMAL',
  'AMAM',
  'AMAO',
  'AMAT',
  'AMBA',
  'AMBC',
  'AMBO',
  'AMBP',
  'AMC',
  'AMCI',
  'AMCR',
  'AMCX',
  'AMD',
  'AME',
  'AMED',
  'AMEH',
  'AMG',
  'AMGN',
  'AMH',
  'AMK',
  'AMKR',
  'AMLX',
  'AMN',
  'AMNB',
  'AMOT',
  'AMOV',
  'AMP',
  'AMPE',
  'AMPG',
  'AMPH',
  'AMPI',
  'AMPL',
  'AMPS',
  'AMPY',
  'AMR',
  'AMRC',
  'AMRK',
  'AMRN',
  'AMRS',
  'AMRX',
  'AMS',
  'AMSC',
  'AMSF',
  'AMST',
  'AMSW',
  'AMT',
  'AMTB',
  'AMTD',
  'AMTI',
  'AMTX',
  'AMWD',
  'AMWL',
  'AMX',
  'AMYT',
  'AMZN',
  'AN',
  'ANAB',
  'ANAC',
  'ANDE',
  'ANEB',
  'ANET',
  'ANF',
  'ANGH',
  'ANGI',
  'ANGN',
  'ANGO',
  'ANIK',
  'ANIP',
  'ANIX',
  'ANNX',
  'ANPC',
  'ANSS',
  'ANTE',
  'ANTX',
  'ANVS',
  'ANY',
  'ANZU',
  'AOGO',
  'AOMR',
  'AON',
  'AORT',
  'AOS',
  'AOSL',
  'AOUT',
  'AP',
  'APA',
  'APAC',
  'APAM',
  'APCA',
  'APCX',
  'APD',
  'APDN',
  'APEI',
  'APEN',
  'APG',
  'APGB',
  'APH',
  'API',
  'APLE',
  'APLS',
  'APLT',
  'APM',
  'APMI',
  'APN',
  'APO',
  'APOG',
  'APP',
  'APPF',
  'APPH',
  'APPN',
  'APPS',
  'APRE',
  'APRN',
  'APT',
  'APTM',
  'APTO',
  'APTV',
  'APTX',
  'APVO',
  'APWC',
  'APXI',
  'APYX',
  'AQB',
  'AQMS',
  'AQN',
  'AQST',
  'AQUA',
  'AR',
  'ARAV',
  'ARAY',
  'ARBE',
  'ARBG',
  'ARBK',
  'ARC',
  'ARCB',
  'ARCC',
  'ARCE',
  'ARCH',
  'ARCK',
  'ARCO',
  'ARCT',
  'ARDS',
  'ARDX',
  'ARE',
  'AREC',
  'AREN',
  'ARES',
  'ARGO',
  'ARGU',
  'ARGX',
  'ARHS',
  'ARI',
  'ARIS',
  'ARIZ',
  'ARKO',
  'ARKR',
  'ARL',
  'ARLO',
  'ARLP',
  'ARMK',
  'ARMP',
  'ARNC',
  'AROC',
  'AROW',
  'ARQQ',
  'ARQT',
  'ARR',
  'ARRW',
  'ARRY',
  'ARTE',
  'ARTL',
  'ARTN',
  'ARTW',
  'ARVL',
  'ARVN',
  'ARW',
  'ARWR',
  'ARYD',
  'ARYE',
  'ASA',
  'ASAI',
  'ASAN',
  'ASAQ',
  'ASAX',
  'ASB',
  'ASC',
  'ASCA',
  'ASCB',
  'ASGN',
  'ASH',
  'ASIX',
  'ASLE',
  'ASLN',
  'ASM',
  'ASMB',
  'ASML',
  'ASND',
  'ASNS',
  'ASO',
  'ASPA',
  'ASPC',
  'ASPN',
  'ASPS',
  'ASPU',
  'ASR',
  'ASRT',
  'ASRV',
  'ASTC',
  'ASTE',
  'ASTL',
  'ASTR',
  'ASTS',
  'ASUR',
  'ASX',
  'ASXC',
  'ASYS',
  'ASZ',
  'ATA',
  'ATAI',
  'ATAK',
  'ATAQ',
  'ATAX',
  'ATC',
  'ATCO',
  'ATCX',
  'ATEC',
  'ATEK',
  'ATEN',
  'ATER',
  'ATEX',
  'ATGE',
  'ATHA',
  'ATHE',
  'ATHM',
  'ATHX',
  'ATI',
  'ATIF',
  'ATIP',
  'ATKR',
  'ATLC',
  'ATLO',
  'ATNF',
  'ATNI',
  'ATNM',
  'ATNX',
  'ATO',
  'ATOM',
  'ATOS',
  'ATR',
  'ATRA',
  'ATRC',
  'ATRI',
  'ATRO',
  'ATSG',
  'ATTO',
  'ATUS',
  'ATVC',
  'ATVI',
  'ATXI',
  'ATXS',
  'ATY',
  'AU',
  'AUB',
  'AUBN',
  'AUD',
  'AUDC',
  'AUGX',
  'AUID',
  'AUMN',
  'AUPH',
  'AUR',
  'AURA',
  'AURC',
  'AUS',
  'AUST',
  'AUTL',
  'AUTO',
  'AUUD',
  'AUVI',
  'AUY',
  'AVA',
  'AVAC',
  'AVAH',
  'AVAL',
  'AVAN',
  'AVAV',
  'AVB',
  'AVCO',
  'AVCT',
  'AVD',
  'AVDL',
  'AVDX',
  'AVEO',
  'AVGO',
  'AVGR',
  'AVHI',
  'AVID',
  'AVIR',
  'AVLR',
  'AVNS',
  'AVNT',
  'AVNW',
  'AVO',
  'AVPT',
  'AVRO',
  'AVT',
  'AVTE',
  'AVTR',
  'AVTX',
  'AVXL',
  'AVY',
  'AVYA',
  'AWH',
  'AWI',
  'AWK',
  'AWR',
  'AWRE',
  'AWX',
  'AX',
  'AXAC',
  'AXDX',
  'AXGN',
  'AXH',
  'AXL',
  'AXLA',
  'AXNX',
  'AXON',
  'AXP',
  'AXR',
  'AXS',
  'AXSM',
  'AXTA',
  'AXTI',
  'AXU',
  'AY',
  'AYI',
  'AYLA',
  'AYRO',
  'AYTU',
  'AYX',
  'AZ',
  'AZEK',
  'AZN',
  'AZO',
  'AZPN',
  'AZRE',
  'AZTA',
  'AZUL',
  'AZYO',
  'AZZ',
  'B',
  'BA',
  'BABA',
  'BAC',
  'BACA',
  'BAFN',
  'BAH',
  'BAK',
  'BALL',
  'BALY',
  'BAM',
  'BANC',
  'BAND',
  'BANF',
  'BANR',
  'BANX',
  'BAOS',
  'BAP',
  'BARK',
  'BASE',
  'BATL',
  'BAX',
  'BB',
  'BBAI',
  'BBAR',
  'BBBY',
  'BBCP',
  'BBD',
  'BBDC',
  'BBGI',
  'BBI',
  'BBIG',
  'BBIO',
  'BBLG',
  'BBLN',
  'BBQ',
  'BBSI',
  'BBU',
  'BBUC',
  'BBVA',
  'BBW',
  'BBWI',
  'BBY',
  'BC',
  'BCAB',
  'BCAC',
  'BCAN',
  'BCBP',
  'BCC',
  'BCDA',
  'BCE',
  'BCEL',
  'BCH',
  'BCLI',
  'BCML',
  'BCO',
  'BCOR',
  'BCOV',
  'BCOW',
  'BCPC',
  'BCRX',
  'BCS',
  'BCSA',
  'BCSF',
  'BCTX',
  'BCYC',
  'BDC',
  'BDL',
  'BDN',
  'BDSX',
  'BDTX',
  'BDX',
  'BE',
  'BEAM',
  'BEAT',
  'BECN',
  'BEDU',
  'BEEM',
  'BEKE',
  'BELF',
  'BELF',
  'BEN',
  'BENE',
  'BEP',
  'BEPC',
  'BERY',
  'BEST',
  'BF.A',
  'BF.B',
  'BFAC',
  'BFAM',
  'BFC',
  'BFH',
  'BFI',
  'BFIN',
  'BFLY',
  'BFRI',
  'BFS',
  'BFST',
  'BG',
  'BGCP',
  'BGFV',
  'BGI',
  'BGNE',
  'BGRY',
  'BGS',
  'BGSF',
  'BGSX',
  'BGXX',
  'BH',
  'BH.A',
  'BHAC',
  'BHAT',
  'BHB',
  'BHC',
  'BHE',
  'BHF',
  'BHG',
  'BHIL',
  'BHLB',
  'BHP',
  'BHR',
  'BHSE',
  'BHVN',
  'BIDU',
  'BIG',
  'BIGC',
  'BIIB',
  'BILI',
  'BILL',
  'BIMI',
  'BIO',
  'BIOC',
  'BIOL',
  'BIOR',
  'BIOS',
  'BIOT',
  'BIOX',
  'BIP',
  'BIPC',
  'BIRD',
  'BITE',
  'BITF',
  'BIVI',
  'BJ',
  'BJDX',
  'BJRI',
  'BK',
  'BKCC',
  'BKD',
  'BKE',
  'BKEP',
  'BKH',
  'BKI',
  'BKKT',
  'BKNG',
  'BKR',
  'BKSC',
  'BKSY',
  'BKTI',
  'BKU',
  'BKYI',
  'BL',
  'BLBD',
  'BLBX',
  'BLCM',
  'BLCO',
  'BLCT',
  'BLD',
  'BLDE',
  'BLDP',
  'BLDR',
  'BLEU',
  'BLFS',
  'BLFY',
  'BLI',
  'BLIN',
  'BLK',
  'BLKB',
  'BLMN',
  'BLND',
  'BLNG',
  'BLNK',
  'BLPH',
  'BLRX',
  'BLSA',
  'BLTE',
  'BLTS',
  'BLU',
  'BLUA',
  'BLUE',
  'BLX',
  'BLZE',
  'BMA',
  'BMAC',
  'BMAQ',
  'BMBL',
  'BMEA',
  'BMI',
  'BMO',
  'BMRA',
  'BMRC',
  'BMRN',
  'BMTX',
  'BMY',
  'BNED',
  'BNFT',
  'BNGO',
  'BNIX',
  'BNL',
  'BNNR',
  'BNOX',
  'BNR',
  'BNRG',
  'BNS',
  'BNSO',
  'BNTC',
  'BNTX',
  'BOAC',
  'BOAS',
  'BOC',
  'BOCN',
  'BODY',
  'BOH',
  'BOKF',
  'BOLT',
  'BON',
  'BOOM',
  'BOOT',
  'BORR',
  'BOSC',
  'BOTJ',
  'BOWL',
  'BOX',
  'BOXD',
  'BOXL',
  'BP',
  'BPAC',
  'BPMC',
  'BPOP',
  'BPRN',
  'BPT',
  'BPTH',
  'BPTS',
  'BQ',
  'BR',
  'BRAC',
  'BRAG',
  'BRBR',
  'BRBS',
  'BRC',
  'BRCC',
  'BRCN',
  'BRD',
  'BRDG',
  'BRDS',
  'BREZ',
  'BRFH',
  'BRFS',
  'BRG',
  'BRID',
  'BRIV',
  'BRK.',
  'BRK.',
  'BRKH',
  'BRKL',
  'BRKR',
  'BRLI',
  'BRLT',
  'BRMK',
  'BRN',
  'BRO',
  'BROG',
  'BROS',
  'BRP',
  'BRPM',
  'BRQS',
  'BRSP',
  'BRT',
  'BRTX',
  'BRX',
  'BRY',
  'BRZE',
  'BSAC',
  'BSAQ',
  'BSBK',
  'BSBR',
  'BSET',
  'BSFC',
  'BSGA',
  'BSGM',
  'BSIG',
  'BSKY',
  'BSM',
  'BSMX',
  'BSQR',
  'BSRR',
  'BSVN',
  'BSX',
  'BSY',
  'BTAI',
  'BTB',
  'BTBD',
  'BTBT',
  'BTCM',
  'BTCS',
  'BTCY',
  'BTG',
  'BTI',
  'BTMD',
  'BTN',
  'BTOG',
  'BTRS',
  'BTTR',
  'BTTX',
  'BTU',
  'BTWN',
  'BTX',
  'BUD',
  'BUR',
  'BURL',
  'BUSE',
  'BV',
  'BVH',
  'BVN',
  'BVS',
  'BVXV',
  'BW',
  'BWA',
  'BWAC',
  'BWAQ',
  'BWAY',
  'BWB',
  'BWC',
  'BWEN',
  'BWFG',
  'BWMN',
  'BWMX',
  'BWV',
  'BWXT',
  'BX',
  'BXC',
  'BXMT',
  'BXP',
  'BXRX',
  'BY',
  'BYD',
  'BYFC',
  'BYN',
  'BYND',
  'BYNO',
  'BYRN',
  'BYSI',
  'BYTS',
  'BZ',
  'BZFD',
  'BZH',
  'BZUN',
  'C',
  'CAAP',
  'CAAS',
  'CABA',
  'CABO',
  'CAC',
  'CACC',
  'CACI',
  'CADE',
  'CADL',
  'CAE',
  'CAG',
  'CAH',
  'CAJ',
  'CAKE',
  'CAL',
  'CALA',
  'CALB',
  'CALM',
  'CALT',
  'CALX',
  'CAMP',
  'CAMT',
  'CAN',
  'CANF',
  'CANG',
  'CANO',
  'CAPL',
  'CAPR',
  'CAR',
  'CARA',
  'CARE',
  'CARG',
  'CARR',
  'CARS',
  'CARV',
  'CASA',
  'CASH',
  'CASI',
  'CASS',
  'CASY',
  'CAT',
  'CATC',
  'CATO',
  'CATY',
  'CB',
  'CBAN',
  'CBAT',
  'CBAY',
  'CBD',
  'CBFV',
  'CBIO',
  'CBL',
  'CBNK',
  'CBOE',
  'CBRE',
  'CBRG',
  'CBRL',
  'CBSH',
  'CBT',
  'CBTX',
  'CBU',
  'CBZ',
  'CC',
  'CCAI',
  'CCAP',
  'CCB',
  'CCBG',
  'CCCC',
  'CCCS',
  'CCEL',
  'CCEP',
  'CCF',
  'CCI',
  'CCJ',
  'CCK',
  'CCL',
  'CCLP',
  'CCM',
  'CCNC',
  'CCNE',
  'CCO',
  'CCOI',
  'CCRD',
  'CCRN',
  'CCS',
  'CCSI',
  'CCTS',
  'CCU',
  'CCV',
  'CCVI',
  'CCXI',
  'CD',
  'CDAK',
  'CDAQ',
  'CDAY',
  'CDE',
  'CDEV',
  'CDLX',
  'CDMO',
  'CDNA',
  'CDNS',
  'CDR',
  'CDRE',
  'CDRO',
  'CDTX',
  'CDW',
  'CDXC',
  'CDXS',
  'CDZI',
  'CE',
  'CEA',
  'CECE',
  'CEG',
  'CEI',
  'CEIX',
  'CELC',
  'CELH',
  'CELU',
  'CELZ',
  'CEMI',
  'CENN',
  'CENQ',
  'CENT',
  'CENT',
  'CENX',
  'CEPU',
  'CEQP',
  'CERE',
  'CERS',
  'CERT',
  'CET',
  'CETX',
  'CEVA',
  'CF',
  'CFB',
  'CFBK',
  'CFFE',
  'CFFI',
  'CFFN',
  'CFFS',
  'CFG',
  'CFIV',
  'CFLT',
  'CFMS',
  'CFR',
  'CFRX',
  'CFSB',
  'CFVI',
  'CG',
  'CGA',
  'CGAU',
  'CGBD',
  'CGC',
  'CGEM',
  'CGEN',
  'CGNT',
  'CGNX',
  'CGRN',
  'CGTX',
  'CHAA',
  'CHCI',
  'CHCO',
  'CHCT',
  'CHD',
  'CHDN',
  'CHE',
  'CHEA',
  'CHEF',
  'CHEK',
  'CHGG',
  'CHH',
  'CHK',
  'CHKP',
  'CHMG',
  'CHMI',
  'CHNG',
  'CHNR',
  'CHPT',
  'CHRA',
  'CHRD',
  'CHRS',
  'CHRW',
  'CHS',
  'CHT',
  'CHTR',
  'CHUY',
  'CHWA',
  'CHWY',
  'CHX',
  'CI',
  'CIA',
  'CIAN',
  'CIB',
  'CIDM',
  'CIEN',
  'CIFR',
  'CIG',
  'CIGI',
  'CIH',
  'CIIG',
  'CIM',
  'CINC',
  'CINF',
  'CING',
  'CINT',
  'CIO',
  'CION',
  'CIR',
  'CISO',
  'CITE',
  'CIVB',
  'CIVI',
  'CIX',
  'CIXX',
  'CIZN',
  'CJJD',
  'CKPT',
  'CKX',
  'CL',
  'CLAA',
  'CLAQ',
  'CLAR',
  'CLAS',
  'CLAY',
  'CLB',
  'CLBK',
  'CLBR',
  'CLBS',
  'CLBT',
  'CLDT',
  'CLDX',
  'CLEU',
  'CLF',
  'CLFD',
  'CLGN',
  'CLH',
  'CLIM',
  'CLIN',
  'CLIR',
  'CLLS',
  'CLMT',
  'CLNE',
  'CLNN',
  'CLOE',
  'CLOV',
  'CLPR',
  'CLPS',
  'CLPT',
  'CLR',
  'CLRB',
  'CLRC',
  'CLRM',
  'CLRO',
  'CLS',
  'CLSD',
  'CLSK',
  'CLSN',
  'CLST',
  'CLVR',
  'CLVS',
  'CLVT',
  'CLW',
  'CLWT',
  'CLX',
  'CLXT',
  'CM',
  'CMA',
  'CMAX',
  'CMBM',
  'CMC',
  'CMCA',
  'CMCL',
  'CMCM',
  'CMCO',
  'CMCS',
  'CMCT',
  'CME',
  'CMG',
  'CMI',
  'CMLS',
  'CMMB',
  'CMP',
  'CMPO',
  'CMPR',
  'CMPS',
  'CMPX',
  'CMRA',
  'CMRE',
  'CMRX',
  'CMS',
  'CMT',
  'CMTG',
  'CMTL',
  'CNA',
  'CNC',
  'CNCE',
  'CND',
  'CNDA',
  'CNDB',
  'CNDT',
  'CNET',
  'CNEY',
  'CNF',
  'CNFR',
  'CNGL',
  'CNHI',
  'CNI',
  'CNK',
  'CNM',
  'CNMD',
  'CNNB',
  'CNNE',
  'CNO',
  'CNOB',
  'CNP',
  'CNQ',
  'CNR',
  'CNS',
  'CNSL',
  'CNSP',
  'CNTA',
  'CNTB',
  'CNTG',
  'CNTQ',
  'CNTX',
  'CNTY',
  'CNVY',
  'CNX',
  'CNXC',
  'CNXN',
  'CO',
  'COCO',
  'COCP',
  'CODA',
  'CODI',
  'CODX',
  'COE',
  'COF',
  'COFS',
  'COGT',
  'COHN',
  'COHU',
  'COIN',
  'COKE',
  'COLB',
  'COLD',
  'COLI',
  'COLL',
  'COLM',
  'COMM',
  'COMP',
  'COMS',
  'CONN',
  'CONX',
  'COO',
  'COOK',
  'COOL',
  'COOP',
  'COP',
  'CORR',
  'CORS',
  'CORT',
  'CORZ',
  'COSM',
  'COST',
  'COTY',
  'COUP',
  'COUR',
  'COVA',
  'COWN',
  'CP',
  'CPA',
  'CPAA',
  'CPAC',
  'CPAQ',
  'CPAR',
  'CPB',
  'CPE',
  'CPF',
  'CPG',
  'CPHC',
  'CPHI',
  'CPIX',
  'CPK',
  'CPLP',
  'CPNG',
  'CPOP',
  'CPRI',
  'CPRT',
  'CPRX',
  'CPS',
  'CPSH',
  'CPSI',
  'CPSS',
  'CPT',
  'CPTK',
  'CPTN',
  'CPUH',
  'CQP',
  'CR',
  'CRAI',
  'CRBP',
  'CRBU',
  'CRC',
  'CRCT',
  'CRD.',
  'CRD.',
  'CRDF',
  'CRDL',
  'CRDO',
  'CREC',
  'CREG',
  'CRES',
  'CREX',
  'CRGE',
  'CRGY',
  'CRH',
  'CRHC',
  'CRI',
  'CRIS',
  'CRK',
  'CRKN',
  'CRL',
  'CRM',
  'CRMD',
  'CRMT',
  'CRNC',
  'CRNT',
  'CRNX',
  'CRON',
  'CROX',
  'CRS',
  'CRSP',
  'CRSR',
  'CRT',
  'CRTD',
  'CRTO',
  'CRTX',
  'CRU',
  'CRUS',
  'CRVL',
  'CRVS',
  'CRWD',
  'CRWS',
  'CRXT',
  'CRZN',
  'CS',
  'CSBR',
  'CSCO',
  'CSCW',
  'CSGP',
  'CSGS',
  'CSII',
  'CSIQ',
  'CSL',
  'CSLM',
  'CSPI',
  'CSR',
  'CSSE',
  'CSTA',
  'CSTE',
  'CSTL',
  'CSTM',
  'CSTR',
  'CSV',
  'CSWC',
  'CSWI',
  'CSX',
  'CTAQ',
  'CTAS',
  'CTBI',
  'CTEK',
  'CTG',
  'CTGO',
  'CTHR',
  'CTIB',
  'CTIC',
  'CTKB',
  'CTLP',
  'CTLT',
  'CTMX',
  'CTO',
  'CTOS',
  'CTRA',
  'CTRE',
  'CTRM',
  'CTRN',
  'CTS',
  'CTSH',
  'CTSO',
  'CTT',
  'CTV',
  'CTVA',
  'CTXR',
  'CTXS',
  'CUBE',
  'CUBI',
  'CUE',
  'CUEN',
  'CUK',
  'CULL',
  'CULP',
  'CURI',
  'CURO',
  'CURV',
  'CUTR',
  'CUZ',
  'CVAC',
  'CVBF',
  'CVCO',
  'CVCY',
  'CVE',
  'CVEO',
  'CVET',
  'CVGI',
  'CVGW',
  'CVI',
  'CVII',
  'CVLG',
  'CVLT',
  'CVLY',
  'CVM',
  'CVNA',
  'CVR',
  'CVRX',
  'CVS',
  'CVT',
  'CVV',
  'CVX',
  'CW',
  'CWAN',
  'CWBC',
  'CWBR',
  'CWCO',
  'CWEN',
  'CWEN',
  'CWH',
  'CWK',
  'CWST',
  'CWT',
  'CX',
  'CXAC',
  'CXDO',
  'CXM',
  'CXW',
  'CYAD',
  'CYAN',
  'CYBE',
  'CYBN',
  'CYBR',
  'CYCC',
  'CYCN',
  'CYD',
  'CYH',
  'CYN',
  'CYRN',
  'CYRX',
  'CYT',
  'CYTH',
  'CYTK',
  'CYTO',
  'CYXT',
  'CZNC',
  'CZOO',
  'CZR',
  'CZWI',
  'D',
  'DAC',
  'DADA',
  'DAIO',
  'DAKT',
  'DAL',
  'DALN',
  'DALS',
  'DAN',
  'DAO',
  'DAOO',
  'DAR',
  'DARE',
  'DASH',
  'DATS',
  'DAVA',
  'DAVE',
  'DAWN',
  'DB',
  'DBD',
  'DBGI',
  'DBI',
  'DBRG',
  'DBTX',
  'DBVT',
  'DBX',
  'DC',
  'DCBO',
  'DCFC',
  'DCGO',
  'DCI',
  'DCO',
  'DCOM',
  'DCP',
  'DCPH',
  'DCRD',
  'DCT',
  'DCTH',
  'DD',
  'DDD',
  'DDI',
  'DDL',
  'DDOG',
  'DDS',
  'DE',
  'DEA',
  'DECA',
  'DECK',
  'DEI',
  'DELL',
  'DEN',
  'DENN',
  'DEO',
  'DERM',
  'DESP',
  'DFFN',
  'DFH',
  'DFIN',
  'DFS',
  'DG',
  'DGIC',
  'DGIC',
  'DGII',
  'DGLY',
  'DGNU',
  'DGX',
  'DH',
  'DHAC',
  'DHBC',
  'DHC',
  'DHCA',
  'DHHC',
  'DHI',
  'DHIL',
  'DHR',
  'DHT',
  'DHX',
  'DIBS',
  'DICE',
  'DILA',
  'DIN',
  'DINO',
  'DIOD',
  'DIS',
  'DISA',
  'DISH',
  'DIT',
  'DJCO',
  'DK',
  'DKDC',
  'DKL',
  'DKNG',
  'DKS',
  'DLA',
  'DLB',
  'DLCA',
  'DLHC',
  'DLNG',
  'DLO',
  'DLPN',
  'DLR',
  'DLTH',
  'DLTR',
  'DLX',
  'DM',
  'DMAC',
  'DMAQ',
  'DMLP',
  'DMRC',
  'DMS',
  'DMTK',
  'DMYS',
  'DNA',
  'DNAA',
  'DNAB',
  'DNAC',
  'DNAD',
  'DNAY',
  'DNB',
  'DNLI',
  'DNMR',
  'DNN',
  'DNOW',
  'DNUT',
  'DNZ',
  'DO',
  'DOC',
  'DOCN',
  'DOCS',
  'DOCU',
  'DOGZ',
  'DOLE',
  'DOMA',
  'DOMO',
  'DOOO',
  'DOOR',
  'DORM',
  'DOUG',
  'DOV',
  'DOW',
  'DOX',
  'DOYU',
  'DPCS',
  'DPRO',
  'DPSI',
  'DPZ',
  'DQ',
  'DRAY',
  'DRCT',
  'DRD',
  'DRE',
  'DRH',
  'DRI',
  'DRIO',
  'DRMA',
  'DRQ',
  'DRRX',
  'DRTS',
  'DRTT',
  'DRUG',
  'DRVN',
  'DS',
  'DSAC',
  'DSAQ',
  'DSEY',
  'DSGN',
  'DSGR',
  'DSGX',
  'DSKE',
  'DSP',
  'DSS',
  'DSWL',
  'DSX',
  'DT',
  'DTC',
  'DTE',
  'DTEA',
  'DTIL',
  'DTM',
  'DTOC',
  'DTRT',
  'DTSS',
  'DTST',
  'DUET',
  'DUK',
  'DUNE',
  'DUO',
  'DUOL',
  'DUOT',
  'DV',
  'DVA',
  'DVAX',
  'DVN',
  'DWAC',
  'DWIN',
  'DWSN',
  'DX',
  'DXC',
  'DXCM',
  'DXF',
  'DXLG',
  'DXPE',
  'DXR',
  'DXYN',
  'DY',
  'DYAI',
  'DYN',
  'DYNT',
  'DZSI',
  'E',
  'EA',
  'EAC',
  'EAF',
  'EAR',
  'EARN',
  'EAST',
  'EAT',
  'EB',
  'EBAC',
  'EBAY',
  'EBC',
  'EBET',
  'EBF',
  'EBIX',
  'EBMT',
  'EBON',
  'EBR',
  'EBS',
  'EBTC',
  'EC',
  'ECC',
  'ECL',
  'ECOM',
  'ECOR',
  'ECPG',
  'ECVT',
  'ED',
  'EDAP',
  'EDBL',
  'EDIT',
  'EDN',
  'EDNC',
  'EDR',
  'EDRY',
  'EDSA',
  'EDTK',
  'EDTX',
  'EDU',
  'EDUC',
  'EE',
  'EEFT',
  'EEIQ',
  'EEX',
  'EFC',
  'EFOI',
  'EFSC',
  'EFTR',
  'EFX',
  'EGAN',
  'EGBN',
  'EGGF',
  'EGHT',
  'EGIO',
  'EGLE',
  'EGLX',
  'EGO',
  'EGP',
  'EGRX',
  'EGY',
  'EH',
  'EHAB',
  'EHC',
  'EHTH',
  'EIC',
  'EIG',
  'EIGR',
  'EIX',
  'EJH',
  'EKSO',
  'EL',
  'ELA',
  'ELAN',
  'ELBM',
  'ELDN',
  'ELEV',
  'ELF',
  'ELLO',
  'ELMD',
  'ELOX',
  'ELP',
  'ELS',
  'ELSE',
  'ELTK',
  'ELV',
  'ELVT',
  'ELY',
  'ELYM',
  'ELYS',
  'EM',
  'EMAN',
  'EMBC',
  'EMBK',
  'EMCF',
  'EME',
  'EMKR',
  'EML',
  'EMLD',
  'EMN',
  'EMR',
  'EMWP',
  'EMX',
  'ENB',
  'ENCP',
  'ENDP',
  'ENER',
  'ENFN',
  'ENG',
  'ENIC',
  'ENJY',
  'ENLC',
  'ENLV',
  'ENOB',
  'ENOV',
  'ENPC',
  'ENPH',
  'ENR',
  'ENS',
  'ENSC',
  'ENSG',
  'ENSV',
  'ENTA',
  'ENTF',
  'ENTG',
  'ENTX',
  'ENV',
  'ENVA',
  'ENVB',
  'ENVX',
  'ENZ',
  'EOCW',
  'EOG',
  'EOLS',
  'EOSE',
  'EP',
  'EPAC',
  'EPAM',
  'EPC',
  'EPD',
  'EPHY',
  'EPIX',
  'EPM',
  'EPR',
  'EPRT',
  'EPSN',
  'EPWR',
  'EPZM',
  'EQ',
  'EQBK',
  'EQC',
  'EQD',
  'EQH',
  'EQHA',
  'EQIX',
  'EQNR',
  'EQOS',
  'EQR',
  'EQRX',
  'EQS',
  'EQT',
  'EQX',
  'ERAS',
  'ERES',
  'ERF',
  'ERIC',
  'ERIE',
  'ERII',
  'ERJ',
  'ERO',
  'ERYP',
  'ES',
  'ESAB',
  'ESAC',
  'ESBA',
  'ESCA',
  'ESE',
  'ESEA',
  'ESGR',
  'ESI',
  'ESLT',
  'ESM',
  'ESMT',
  'ESNT',
  'ESP',
  'ESPR',
  'ESQ',
  'ESRT',
  'ESS',
  'ESSA',
  'ESSC',
  'ESTA',
  'ESTC',
  'ESTE',
  'ET',
  'ETAC',
  'ETD',
  'ETN',
  'ETNB',
  'ETON',
  'ETR',
  'ETRN',
  'ETSY',
  'ETTX',
  'ETWO',
  'EUCR',
  'EURN',
  'EVA',
  'EVAX',
  'EVBG',
  'EVBN',
  'EVC',
  'EVCM',
  'EVE',
  'EVER',
  'EVEX',
  'EVFM',
  'EVGN',
  'EVGO',
  'EVGR',
  'EVH',
  'EVI',
  'EVK',
  'EVLO',
  'EVLV',
  'EVO',
  'EVOJ',
  'EVOK',
  'EVOL',
  'EVOP',
  'EVR',
  'EVRG',
  'EVRI',
  'EVTC',
  'EVTL',
  'EW',
  'EWBC',
  'EWCZ',
  'EWTX',
  'EXAI',
  'EXAS',
  'EXC',
  'EXEL',
  'EXFY',
  'EXK',
  'EXLS',
  'EXN',
  'EXP',
  'EXPD',
  'EXPE',
  'EXPI',
  'EXPO',
  'EXPR',
  'EXR',
  'EXTN',
  'EXTR',
  'EYE',
  'EYEN',
  'EYES',
  'EYPT',
  'EZFL',
  'EZGO',
  'EZPW',
  'F',
  'FA',
  'FACA',
  'FACT',
  'FAF',
  'FAMI',
  'FANG',
  'FANH',
  'FARM',
  'FARO',
  'FAST',
  'FAT',
  'FATB',
  'FATE',
  'FATH',
  'FATP',
  'FBC',
  'FBHS',
  'FBIO',
  'FBIZ',
  'FBK',
  'FBMS',
  'FBNC',
  'FBP',
  'FBRT',
  'FBRX',
  'FC',
  'FCAP',
  'FCAX',
  'FCBC',
  'FCCO',
  'FCEL',
  'FCF',
  'FCFS',
  'FCN',
  'FCNC',
  'FCPT',
  'FCRD',
  'FCUV',
  'FCX',
  'FDBC',
  'FDMT',
  'FDP',
  'FDS',
  'FDUS',
  'FDX',
  'FE',
  'FEAM',
  'FEDU',
  'FEIM',
  'FELE',
  'FEMY',
  'FENC',
  'FENG',
  'FERG',
  'FET',
  'FEXD',
  'FF',
  'FFBC',
  'FFBW',
  'FFHL',
  'FFIC',
  'FFIE',
  'FFIN',
  'FFIV',
  'FFNW',
  'FFWM',
  'FGBI',
  'FGEN',
  'FGF',
  'FGI',
  'FGMC',
  'FHB',
  'FHI',
  'FHLT',
  'FHN',
  'FHS',
  'FHTX',
  'FIAC',
  'FIBK',
  'FICO',
  'FICV',
  'FIGS',
  'FINM',
  'FINV',
  'FINW',
  'FIS',
  'FISI',
  'FISV',
  'FITB',
  'FIVE',
  'FIVN',
  'FIX',
  'FIXX',
  'FIZZ',
  'FKWL',
  'FL',
  'FLAC',
  'FLAG',
  'FLEX',
  'FLFV',
  'FLGC',
  'FLGT',
  'FLIC',
  'FLL',
  'FLME',
  'FLNC',
  'FLNG',
  'FLNT',
  'FLO',
  'FLR',
  'FLS',
  'FLT',
  'FLUX',
  'FLWS',
  'FLXS',
  'FLYA',
  'FLYW',
  'FMAO',
  'FMBH',
  'FMC',
  'FMIV',
  'FMNB',
  'FMS',
  'FMTX',
  'FMX',
  'FN',
  'FNA',
  'FNB',
  'FNCB',
  'FNCH',
  'FND',
  'FNF',
  'FNGR',
  'FNHC',
  'FNKO',
  'FNLC',
  'FNV',
  'FNVT',
  'FNWB',
  'FNWD',
  'FOA',
  'FOCS',
  'FOLD',
  'FONR',
  'FOR',
  'FORA',
  'FORD',
  'FORG',
  'FORM',
  'FORR',
  'FORT',
  'FOSL',
  'FOUN',
  'FOUR',
  'FOX',
  'FOXA',
  'FOXF',
  'FOXW',
  'FPAC',
  'FPAY',
  'FPH',
  'FPI',
  'FR',
  'FRAF',
  'FRBA',
  'FRBK',
  'FRBN',
  'FRC',
  'FRD',
  'FREE',
  'FREQ',
  'FREY',
  'FRG',
  'FRGE',
  'FRGI',
  'FRGT',
  'FRHC',
  'FRLA',
  'FRLN',
  'FRME',
  'FRO',
  'FROG',
  'FRON',
  'FRPH',
  'FRPT',
  'FRSG',
  'FRSH',
  'FRST',
  'FRSX',
  'FRT',
  'FRW',
  'FRXB',
  'FSBC',
  'FSBW',
  'FSEA',
  'FSFG',
  'FSI',
  'FSK',
  'FSLR',
  'FSLY',
  'FSM',
  'FSNB',
  'FSP',
  'FSR',
  'FSRD',
  'FSRX',
  'FSS',
  'FSSI',
  'FST',
  'FSTR',
  'FSTX',
  'FSV',
  'FTAA',
  'FTAI',
  'FTCH',
  'FTCI',
  'FTCV',
  'FTDR',
  'FTEK',
  'FTEV',
  'FTFT',
  'FTHM',
  'FTI',
  'FTII',
  'FTK',
  'FTNT',
  'FTPA',
  'FTRP',
  'FTS',
  'FTV',
  'FTVI',
  'FUBO',
  'FUL',
  'FULC',
  'FULT',
  'FUN',
  'FUNC',
  'FURY',
  'FUSB',
  'FUSN',
  'FUTU',
  'FUV',
  'FVAM',
  'FVCB',
  'FVIV',
  'FVRR',
  'FVT',
  'FWAC',
  'FWBI',
  'FWON',
  'FWP',
  'FWRD',
  'FWRG',
  'FXCO',
  'FXLV',
  'FXNC',
  'FYBR',
  'FZT',
  'G',
  'GABC',
  'GACQ',
  'GAIA',
  'GAIN',
  'GALT',
  'GAMB',
  'GAMC',
  'GAME',
  'GAN',
  'GANX',
  'GAPA',
  'GAQ',
  'GASS',
  'GATE',
  'GATO',
  'GATX',
  'GAU',
  'GB',
  'GBBK',
  'GBCI',
  'GBDC',
  'GBIO',
  'GBL',
  'GBLI',
  'GBNH',
  'GBNY',
  'GBOX',
  'GBR',
  'GBRG',
  'GBS',
  'GBT',
  'GBTG',
  'GBX',
  'GCBC',
  'GCI',
  'GCMG',
  'GCO',
  'GCP',
  'GCTK',
  'GD',
  'GDDY',
  'GDEN',
  'GDEV',
  'GDNR',
  'GDOT',
  'GDRX',
  'GDS',
  'GDST',
  'GDYN',
  'GE',
  'GECC',
  'GEEX',
  'GEF',
  'GEF.',
  'GEG',
  'GEHI',
  'GEL',
  'GENC',
  'GENE',
  'GENI',
  'GENQ',
  'GEO',
  'GEOS',
  'GERN',
  'GES',
  'GET',
  'GEVO',
  'GFAI',
  'GFF',
  'GFGD',
  'GFI',
  'GFL',
  'GFOR',
  'GFS',
  'GFX',
  'GGAA',
  'GGAL',
  'GGB',
  'GGE',
  'GGG',
  'GGMC',
  'GGR',
  'GH',
  'GHAC',
  'GHC',
  'GHG',
  'GHIX',
  'GHL',
  'GHLD',
  'GHM',
  'GHRS',
  'GHSI',
  'GIA',
  'GIAC',
  'GIB',
  'GIC',
  'GIFI',
  'GIGM',
  'GIII',
  'GIIX',
  'GIL',
  'GILD',
  'GILT',
  'GIPR',
  'GIS',
  'GIW',
  'GKOS',
  'GL',
  'GLAD',
  'GLBE',
  'GLBL',
  'GLBS',
  'GLBZ',
  'GLDD',
  'GLDG',
  'GLEE',
  'GLG',
  'GLHA',
  'GLLI',
  'GLMD',
  'GLNG',
  'GLOB',
  'GLOP',
  'GLP',
  'GLPG',
  'GLPI',
  'GLRE',
  'GLS',
  'GLSI',
  'GLSP',
  'GLT',
  'GLTA',
  'GLTO',
  'GLUE',
  'GLW',
  'GLYC',
  'GM',
  'GMAB',
  'GMBL',
  'GMDA',
  'GME',
  'GMED',
  'GMFI',
  'GMGI',
  'GMRE',
  'GMS',
  'GMTX',
  'GMVD',
  'GNAC',
  'GNE',
  'GNFT',
  'GNK',
  'GNL',
  'GNLN',
  'GNPX',
  'GNRC',
  'GNS',
  'GNSS',
  'GNTA',
  'GNTX',
  'GNTY',
  'GNUS',
  'GNW',
  'GO',
  'GOAC',
  'GOBI',
  'GOCO',
  'GOED',
  'GOEV',
  'GOGL',
  'GOGN',
  'GOGO',
  'GOL',
  'GOLD',
  'GOLF',
  'GOOD',
  'GOOG',
  'GOOG',
  'GOOS',
  'GORO',
  'GOSS',
  'GOTU',
  'GOVX',
  'GP',
  'GPAC',
  'GPC',
  'GPCO',
  'GPI',
  'GPK',
  'GPL',
  'GPMT',
  'GPN',
  'GPOR',
  'GPP',
  'GPRE',
  'GPRK',
  'GPRO',
  'GPS',
  'GRAB',
  'GRAY',
  'GRBK',
  'GRC',
  'GRCL',
  'GRCY',
  'GREE',
  'GRFS',
  'GRIL',
  'GRIN',
  'GRMN',
  'GRNA',
  'GRNQ',
  'GROM',
  'GROV',
  'GROW',
  'GROY',
  'GRPH',
  'GRPN',
  'GRTS',
  'GRTX',
  'GRVI',
  'GRVY',
  'GRWG',
  'GS',
  'GSAQ',
  'GSAT',
  'GSBC',
  'GSBD',
  'GSEV',
  'GSHD',
  'GSIT',
  'GSK',
  'GSL',
  'GSM',
  'GSMG',
  'GSQB',
  'GSQD',
  'GSRM',
  'GSUN',
  'GSV',
  'GT',
  'GTAC',
  'GTBP',
  'GTE',
  'GTEC',
  'GTES',
  'GTH',
  'GTHX',
  'GTIM',
  'GTLB',
  'GTLS',
  'GTN',
  'GTN.',
  'GTPA',
  'GTPB',
  'GTX',
  'GTY',
  'GURE',
  'GVA',
  'GVCI',
  'GVP',
  'GWGH',
  'GWH',
  'GWII',
  'GWRE',
  'GWRS',
  'GWW',
  'GXII',
  'GXO',
  'GYRO',
  'H',
  'HA',
  'HAAC',
  'HAE',
  'HAFC',
  'HAIA',
  'HAIN',
  'HAL',
  'HALL',
  'HALO',
  'HAPP',
  'HARP',
  'HAS',
  'HASI',
  'HAYN',
  'HAYW',
  'HBAN',
  'HBB',
  'HBCP',
  'HBI',
  'HBIO',
  'HBM',
  'HBNC',
  'HBT',
  'HCA',
  'HCAR',
  'HCAT',
  'HCC',
  'HCCI',
  'HCDI',
  'HCI',
  'HCIC',
  'HCII',
  'HCKT',
  'HCM',
  'HCMA',
  'HCNE',
  'HCP',
  'HCSG',
  'HCTI',
  'HCVI',
  'HCWB',
  'HD',
  'HDB',
  'HDSN',
  'HE',
  'HEAR',
  'HEES',
  'HEI',
  'HEI.',
  'HELE',
  'HEP',
  'HEPA',
  'HEPS',
  'HERA',
  'HES',
  'HESM',
  'HEXO',
  'HFBL',
  'HFFG',
  'HFWA',
  'HGBL',
  'HGEN',
  'HGTY',
  'HGV',
  'HHC',
  'HHGC',
  'HHLA',
  'HHR',
  'HHS',
  'HI',
  'HIBB',
  'HIFS',
  'HIG',
  'HIGA',
  'HIHO',
  'HII',
  'HIII',
  'HIL',
  'HILS',
  'HIMS',
  'HIMX',
  'HIPO',
  'HITI',
  'HIVE',
  'HIW',
  'HL',
  'HLAH',
  'HLBZ',
  'HLF',
  'HLG',
  'HLGN',
  'HLI',
  'HLIO',
  'HLIT',
  'HLLY',
  'HLMN',
  'HLNE',
  'HLT',
  'HLTH',
  'HLVX',
  'HLX',
  'HMA',
  'HMC',
  'HMCO',
  'HMLP',
  'HMN',
  'HMNF',
  'HMPT',
  'HMST',
  'HMTV',
  'HMY',
  'HNGR',
  'HNI',
  'HNNA',
  'HNRA',
  'HNRG',
  'HNST',
  'HNVR',
  'HOFT',
  'HOFV',
  'HOG',
  'HOLI',
  'HOLX',
  'HOMB',
  'HON',
  'HONE',
  'HOOD',
  'HOOK',
  'HOPE',
  'HORI',
  'HOTH',
  'HOUR',
  'HOUS',
  'HOV',
  'HOWL',
  'HP',
  'HPE',
  'HPK',
  'HPLT',
  'HPP',
  'HPQ',
  'HPX',
  'HQI',
  'HQY',
  'HR',
  'HRB',
  'HRI',
  'HRL',
  'HRMY',
  'HROW',
  'HRT',
  'HRTG',
  'HRTX',
  'HRZN',
  'HSAQ',
  'HSBC',
  'HSC',
  'HSCS',
  'HSDT',
  'HSIC',
  'HSII',
  'HSKA',
  'HSON',
  'HST',
  'HSTM',
  'HSTO',
  'HSY',
  'HT',
  'HTA',
  'HTAQ',
  'HTBI',
  'HTBK',
  'HTCR',
  'HTGC',
  'HTGM',
  'HTH',
  'HTHT',
  'HTLD',
  'HTLF',
  'HTOO',
  'HTPA',
  'HTZ',
  'HUBB',
  'HUBG',
  'HUBS',
  'HUDI',
  'HUGE',
  'HUGS',
  'HUIZ',
  'HUM',
  'HUMA',
  'HUN',
  'HURC',
  'HURN',
  'HUSA',
  'HUT',
  'HUYA',
  'HVBC',
  'HVT',
  'HWBK',
  'HWC',
  'HWEL',
  'HWKN',
  'HWKZ',
  'HWM',
  'HXL',
  'HY',
  'HYFM',
  'HYLN',
  'HYMC',
  'HYPR',
  'HYRE',
  'HYW',
  'HYZN',
  'HZN',
  'HZNP',
  'HZO',
  'HZON',
  'IAA',
  'IAC',
  'IACC',
  'IAG',
  'IART',
  'IAS',
  'IAUX',
  'IBA',
  'IBCP',
  'IBER',
  'IBEX',
  'IBIO',
  'IBKR',
  'IBM',
  'IBN',
  'IBOC',
  'IBP',
  'IBRX',
  'IBTX',
  'ICAD',
  'ICCC',
  'ICCH',
  'ICCM',
  'ICD',
  'ICE',
  'ICFI',
  'ICHR',
  'ICL',
  'ICLK',
  'ICLR',
  'ICMB',
  'ICNC',
  'ICPT',
  'ICUI',
  'ICVX',
  'ID',
  'IDA',
  'IDAI',
  'IDBA',
  'IDCC',
  'IDEX',
  'IDN',
  'IDR',
  'IDRA',
  'IDT',
  'IDW',
  'IDXX',
  'IDYA',
  'IE',
  'IEA',
  'IEP',
  'IESC',
  'IEX',
  'IFBD',
  'IFF',
  'IFIN',
  'IFRX',
  'IFS',
  'IGAC',
  'IGC',
  'IGIC',
  'IGMS',
  'IGNY',
  'IGT',
  'IGTA',
  'IH',
  'IHG',
  'IHRT',
  'IHS',
  'IHT',
  'III',
  'IIII',
  'IIIN',
  'IIIV',
  'IINN',
  'IIPR',
  'IIVI',
  'IKNA',
  'IKT',
  'ILMN',
  'ILPT',
  'IMAB',
  'IMAC',
  'IMAQ',
  'IMAX',
  'IMBI',
  'IMCC',
  'IMCR',
  'IMGN',
  'IMGO',
  'IMH',
  'IMKT',
  'IMMP',
  'IMMR',
  'IMMX',
  'IMNM',
  'IMO',
  'IMOS',
  'IMPL',
  'IMPP',
  'IMPX',
  'IMRA',
  'IMRN',
  'IMRX',
  'IMTE',
  'IMTX',
  'IMUX',
  'IMV',
  'IMVT',
  'IMXI',
  'INAB',
  'INAQ',
  'INBK',
  'INBX',
  'INCR',
  'INCY',
  'INDB',
  'INDI',
  'INDO',
  'INDP',
  'INDT',
  'INFA',
  'INFI',
  'INFN',
  'INFU',
  'INFY',
  'ING',
  'INGN',
  'INGR',
  'INKA',
  'INKT',
  'INM',
  'INMB',
  'INMD',
  'INN',
  'INNV',
  'INO',
  'INOD',
  'INPX',
  'INSE',
  'INSG',
  'INSM',
  'INSP',
  'INST',
  'INSW',
  'INT',
  'INTA',
  'INTC',
  'INTE',
  'INTG',
  'INTR',
  'INTT',
  'INTU',
  'INTZ',
  'INUV',
  'INVA',
  'INVE',
  'INVH',
  'INVO',
  'INVZ',
  'INZY',
  'IOAC',
  'IOBT',
  'IONM',
  'IONQ',
  'IONR',
  'IONS',
  'IOR',
  'IOSP',
  'IOT',
  'IOVA',
  'IP',
  'IPA',
  'IPAR',
  'IPAX',
  'IPDN',
  'IPG',
  'IPGP',
  'IPHA',
  'IPI',
  'IPOD',
  'IPOF',
  'IPSC',
  'IPVA',
  'IPVF',
  'IPVI',
  'IPW',
  'IPWR',
  'IPX',
  'IQ',
  'IQMD',
  'IQV',
  'IR',
  'IRBT',
  'IRDM',
  'IREN',
  'IRIX',
  'IRM',
  'IRMD',
  'IRNT',
  'IROQ',
  'IRRX',
  'IRS',
  'IRT',
  'IRTC',
  'IRWD',
  'IS',
  'ISAA',
  'ISDR',
  'ISEE',
  'ISIG',
  'ISLE',
  'ISO',
  'ISPC',
  'ISPO',
  'ISR',
  'ISRG',
  'ISSC',
  'ISTR',
  'ISUN',
  'IT',
  'ITAQ',
  'ITCB',
  'ITCI',
  'ITGR',
  'ITHX',
  'ITI',
  'ITIC',
  'ITOS',
  'ITP',
  'ITQ',
  'ITRG',
  'ITRI',
  'ITRM',
  'ITRN',
  'ITT',
  'ITUB',
  'ITW',
  'IVA',
  'IVAC',
  'IVC',
  'IVCA',
  'IVCB',
  'IVCP',
  'IVR',
  'IVT',
  'IVZ',
  'IX',
  'IXAQ',
  'IXHL',
  'IZEA',
  'J',
  'JACK',
  'JAGX',
  'JAKK',
  'JAMF',
  'JAN',
  'JANX',
  'JAQC',
  'JATT',
  'JAZZ',
  'JBGS',
  'JBHT',
  'JBI',
  'JBL',
  'JBLU',
  'JBSS',
  'JBT',
  'JCI',
  'JCIC',
  'JCSE',
  'JCTC',
  'JD',
  'JEF',
  'JELD',
  'JFIN',
  'JFU',
  'JG',
  'JGGC',
  'JHG',
  'JHX',
  'JILL',
  'JJSF',
  'JKHY',
  'JKS',
  'JLL',
  'JMAC',
  'JMIA',
  'JMSB',
  'JNCE',
  'JNJ',
  'JNPR',
  'JOAN',
  'JOB',
  'JOBY',
  'JOE',
  'JOFF',
  'JOUT',
  'JPM',
  'JPPY',
  'JRSH',
  'JRVR',
  'JSPR',
  'JT',
  'JUGG',
  'JUN',
  'JUPW',
  'JVA',
  'JWAC',
  'JWEL',
  'JWN',
  'JWSM',
  'JXN',
  'JYAC',
  'JYNT',
  'JZXN',
  'K',
  'KACL',
  'KAHC',
  'KAI',
  'KAII',
  'KAIR',
  'KAL',
  'KALA',
  'KALU',
  'KALV',
  'KAMN',
  'KAR',
  'KARO',
  'KAVL',
  'KB',
  'KBAL',
  'KBH',
  'KBNT',
  'KBR',
  'KC',
  'KCAC',
  'KCGI',
  'KD',
  'KDNY',
  'KDP',
  'KE',
  'KELY',
  'KELY',
  'KEN',
  'KEP',
  'KEQU',
  'KERN',
  'KEX',
  'KEY',
  'KEYS',
  'KFFB',
  'KFRC',
  'KFS',
  'KFY',
  'KGC',
  'KHC',
  'KIDS',
  'KIII',
  'KIM',
  'KIND',
  'KINS',
  'KINZ',
  'KIQ',
  'KIRK',
  'KKR',
  'KLAC',
  'KLAQ',
  'KLIC',
  'KLR',
  'KLTR',
  'KLXE',
  'KMB',
  'KMDA',
  'KMI',
  'KMPH',
  'KMPR',
  'KMT',
  'KMX',
  'KN',
  'KNBE',
  'KNDI',
  'KNOP',
  'KNSA',
  'KNSL',
  'KNSW',
  'KNTE',
  'KNTK',
  'KNX',
  'KO',
  'KOD',
  'KODK',
  'KOF',
  'KOP',
  'KOPN',
  'KORE',
  'KOS',
  'KOSS',
  'KPLT',
  'KPRX',
  'KPTI',
  'KR',
  'KRBP',
  'KRC',
  'KREF',
  'KRG',
  'KRKR',
  'KRMD',
  'KRNL',
  'KRNT',
  'KRNY',
  'KRO',
  'KRON',
  'KROS',
  'KRP',
  'KRT',
  'KRTX',
  'KRUS',
  'KRYS',
  'KSCP',
  'KSI',
  'KSPN',
  'KSS',
  'KT',
  'KTB',
  'KTCC',
  'KTOS',
  'KTRA',
  'KTTA',
  'KUKE',
  'KULR',
  'KURA',
  'KVHI',
  'KVSA',
  'KVSC',
  'KW',
  'KWAC',
  'KWR',
  'KXIN',
  'KYCH',
  'KYMR',
  'KZIA',
  'KZR',
  'L',
  'LAAA',
  'LAB',
  'LABP',
  'LAC',
  'LAD',
  'LADR',
  'LAKE',
  'LAMR',
  'LANC',
  'LAND',
  'LARK',
  'LASR',
  'LATG',
  'LAUR',
  'LAW',
  'LAX',
  'LAZ',
  'LAZR',
  'LAZY',
  'LBAI',
  'LBBB',
  'LBC',
  'LBPH',
  'LBPS',
  'LBRD',
  'LBRD',
  'LBRT',
  'LBTY',
  'LBTY',
  'LBTY',
  'LC',
  'LCA',
  'LCAA',
  'LCFY',
  'LCI',
  'LCID',
  'LCII',
  'LCNB',
  'LCTX',
  'LCUT',
  'LCW',
  'LDHA',
  'LDI',
  'LDOS',
  'LE',
  'LEA',
  'LEAP',
  'LECO',
  'LEDS',
  'LEE',
  'LEG',
  'LEGA',
  'LEGH',
  'LEGN',
  'LEJU',
  'LEN',
  'LEN.',
  'LESL',
  'LEU',
  'LEV',
  'LEVI',
  'LEXX',
  'LFAC',
  'LFC',
  'LFG',
  'LFLY',
  'LFMD',
  'LFST',
  'LFT',
  'LFTR',
  'LFUS',
  'LFVN',
  'LGAC',
  'LGF.',
  'LGF.',
  'LGHL',
  'LGIH',
  'LGL',
  'LGMK',
  'LGND',
  'LGO',
  'LGST',
  'LGTO',
  'LGV',
  'LGVC',
  'LGVN',
  'LH',
  'LHAA',
  'LHC',
  'LHCG',
  'LHDX',
  'LHX',
  'LI',
  'LIAN',
  'LIBY',
  'LICY',
  'LIDR',
  'LIFE',
  'LII',
  'LILA',
  'LILA',
  'LILM',
  'LIN',
  'LINC',
  'LIND',
  'LINK',
  'LION',
  'LIQT',
  'LITB',
  'LITE',
  'LITM',
  'LITT',
  'LIVB',
  'LIVE',
  'LIVN',
  'LIXT',
  'LIZI',
  'LJAQ',
  'LJPC',
  'LKCO',
  'LKFN',
  'LKQ',
  'LL',
  'LLAP',
  'LLL',
  'LLY',
  'LMAC',
  'LMAO',
  'LMAT',
  'LMB',
  'LMDX',
  'LMFA',
  'LMND',
  'LMNL',
  'LMNR',
  'LMPX',
  'LMST',
  'LMT',
  'LNC',
  'LND',
  'LNDC',
  'LNFA',
  'LNG',
  'LNN',
  'LNSR',
  'LNT',
  'LNTH',
  'LNW',
  'LOAN',
  'LOB',
  'LOCC',
  'LOCL',
  'LOCO',
  'LODE',
  'LOGC',
  'LOGI',
  'LOKM',
  'LOMA',
  'LOOP',
  'LOPE',
  'LOTZ',
  'LOV',
  'LOVE',
  'LOW',
  'LPCN',
  'LPG',
  'LPI',
  'LPL',
  'LPLA',
  'LPRO',
  'LPSN',
  'LPTH',
  'LPTX',
  'LPX',
  'LQDA',
  'LQDT',
  'LRCX',
  'LRFC',
  'LRMR',
  'LRN',
  'LSAK',
  'LSBK',
  'LSCC',
  'LSEA',
  'LSF',
  'LSI',
  'LSPD',
  'LSPR',
  'LSTR',
  'LTBR',
  'LTC',
  'LTCH',
  'LTH',
  'LTHM',
  'LTRN',
  'LTRP',
  'LTRP',
  'LTRX',
  'LTRY',
  'LU',
  'LUCD',
  'LULU',
  'LUMN',
  'LUMO',
  'LUNA',
  'LUNG',
  'LUV',
  'LUXA',
  'LVAC',
  'LVLU',
  'LVO',
  'LVOX',
  'LVRA',
  'LVS',
  'LVTX',
  'LW',
  'LWAY',
  'LWLG',
  'LX',
  'LXEH',
  'LXFR',
  'LXP',
  'LXRX',
  'LXU',
  'LYB',
  'LYEL',
  'LYFT',
  'LYG',
  'LYL',
  'LYLT',
  'LYRA',
  'LYT',
  'LYTS',
  'LYV',
  'LZ',
  'LZB',
  'M',
  'MA',
  'MAA',
  'MAAQ',
  'MAC',
  'MACA',
  'MACC',
  'MACK',
  'MAG',
  'MAIN',
  'MAN',
  'MANH',
  'MANT',
  'MANU',
  'MAPS',
  'MAQC',
  'MAR',
  'MARA',
  'MARK',
  'MARP',
  'MAS',
  'MASI',
  'MASS',
  'MAT',
  'MATV',
  'MATW',
  'MATX',
  'MAX',
  'MAXN',
  'MAXR',
  'MAYS',
  'MBAC',
  'MBCN',
  'MBI',
  'MBII',
  'MBIN',
  'MBIO',
  'MBOT',
  'MBRX',
  'MBSC',
  'MBT',
  'MBTC',
  'MBUU',
  'MBWM',
  'MC',
  'MCAA',
  'MCAC',
  'MCAE',
  'MCAF',
  'MCAG',
  'MCB',
  'MCBC',
  'MCBS',
  'MCD',
  'MCFT',
  'MCG',
  'MCHP',
  'MCHX',
  'MCK',
  'MCLD',
  'MCO',
  'MCRB',
  'MCRI',
  'MCS',
  'MCW',
  'MCY',
  'MD',
  'MDB',
  'MDC',
  'MDGL',
  'MDGS',
  'MDH',
  'MDIA',
  'MDJH',
  'MDLZ',
  'MDNA',
  'MDRR',
  'MDRX',
  'MDT',
  'MDU',
  'MDV',
  'MDVL',
  'MDWD',
  'MDWT',
  'MDXG',
  'MDXH',
  'ME',
  'MEAC',
  'MEC',
  'MED',
  'MEDP',
  'MEDS',
  'MEG',
  'MEI',
  'MEIP',
  'MEKA',
  'MELI',
  'MEOA',
  'MEOH',
  'MERC',
  'MESA',
  'MESO',
  'MET',
  'META',
  'METC',
  'METX',
  'MF',
  'MFA',
  'MFC',
  'MFG',
  'MFGP',
  'MFH',
  'MFIN',
  'MG',
  'MGA',
  'MGEE',
  'MGI',
  'MGIC',
  'MGLD',
  'MGM',
  'MGNI',
  'MGNX',
  'MGPI',
  'MGRC',
  'MGTA',
  'MGTX',
  'MGY',
  'MGYR',
  'MHH',
  'MHK',
  'MHLD',
  'MHO',
  'MHUA',
  'MIC',
  'MICS',
  'MICT',
  'MIDD',
  'MIGI',
  'MILE',
  'MIMO',
  'MIND',
  'MINM',
  'MIR',
  'MIRM',
  'MIRO',
  'MIST',
  'MIT',
  'MITA',
  'MITC',
  'MITK',
  'MITO',
  'MITQ',
  'MITT',
  'MIXT',
  'MKC',
  'MKD',
  'MKFG',
  'MKL',
  'MKSI',
  'MKTW',
  'MKTX',
  'ML',
  'MLAB',
  'MLAC',
  'MLAI',
  'MLCO',
  'MLI',
  'MLKN',
  'MLM',
  'MLNK',
  'MLP',
  'MLR',
  'MLSS',
  'MLTX',
  'MLVF',
  'MMAT',
  'MMC',
  'MMI',
  'MMLP',
  'MMM',
  'MMMB',
  'MMP',
  'MMS',
  'MMSI',
  'MMX',
  'MMYT',
  'MN',
  'MNDO',
  'MNDT',
  'MNDY',
  'MNKD',
  'MNMD',
  'MNOV',
  'MNPR',
  'MNRL',
  'MNRO',
  'MNSB',
  'MNSO',
  'MNST',
  'MNTK',
  'MNTN',
  'MNTS',
  'MNTV',
  'MNTX',
  'MO',
  'MOBQ',
  'MOD',
  'MODN',
  'MODV',
  'MOFG',
  'MOG.',
  'MOG.',
  'MOGO',
  'MOGU',
  'MOH',
  'MOHO',
  'MOLN',
  'MOMO',
  'MON',
  'MOR',
  'MORF',
  'MORN',
  'MOS',
  'MOTS',
  'MOV',
  'MOVE',
  'MOXC',
  'MP',
  'MPAA',
  'MPAC',
  'MPB',
  'MPC',
  'MPLN',
  'MPLX',
  'MPRA',
  'MPW',
  'MPWR',
  'MPX',
  'MQ',
  'MRAI',
  'MRAM',
  'MRBK',
  'MRC',
  'MRCC',
  'MRCY',
  'MREO',
  'MRIN',
  'MRK',
  'MRKR',
  'MRM',
  'MRNA',
  'MRNS',
  'MRO',
  'MRSN',
  'MRTN',
  'MRTX',
  'MRUS',
  'MRVI',
  'MRVL',
  'MS',
  'MSA',
  'MSAC',
  'MSB',
  'MSBI',
  'MSC',
  'MSCI',
  'MSDA',
  'MSEX',
  'MSFT',
  'MSGE',
  'MSGM',
  'MSGS',
  'MSI',
  'MSM',
  'MSN',
  'MSPR',
  'MSSA',
  'MSTR',
  'MSVB',
  'MT',
  'MTA',
  'MTAC',
  'MTAL',
  'MTB',
  'MTBC',
  'MTC',
  'MTCH',
  'MTCR',
  'MTD',
  'MTDR',
  'MTEK',
  'MTEM',
  'MTEX',
  'MTG',
  'MTH',
  'MTL',
  'MTLS',
  'MTMT',
  'MTN',
  'MTNB',
  'MTOR',
  'MTP',
  'MTR',
  'MTRN',
  'MTRX',
  'MTRY',
  'MTSI',
  'MTTR',
  'MTVC',
  'MTW',
  'MTX',
  'MTZ',
  'MU',
  'MUDS',
  'MUFG',
  'MULN',
  'MUR',
  'MURF',
  'MUSA',
  'MUX',
  'MVBF',
  'MVIS',
  'MVST',
  'MWA',
  'MX',
  'MXC',
  'MXCT',
  'MXL',
  'MYE',
  'MYFW',
  'MYGN',
  'MYMD',
  'MYNA',
  'MYNZ',
  'MYO',
  'MYOV',
  'MYPS',
  'MYRG',
  'MYSZ',
  'MYTE',
  'NAAC',
  'NAAS',
  'NABL',
  'NAII',
  'NAK',
  'NAOV',
  'NAPA',
  'NARI',
  'NAT',
  'NATH',
  'NATI',
  'NATR',
  'NAUT',
  'NAVB',
  'NAVI',
  'NBEV',
  'NBHC',
  'NBIX',
  'NBN',
  'NBR',
  'NBRV',
  'NBSE',
  'NBST',
  'NBTB',
  'NBTX',
  'NBY',
  'NC',
  'NCAC',
  'NCLH',
  'NCMI',
  'NCNA',
  'NCNO',
  'NCR',
  'NCSM',
  'NCTY',
  'NDAC',
  'NDAQ',
  'NDLS',
  'NDRA',
  'NDSN',
  'NE',
  'NECB',
  'NEE',
  'NEGG',
  'NEM',
  'NEN',
  'NEO',
  'NEOG',
  'NEON',
  'NEP',
  'NEPH',
  'NEPT',
  'NERV',
  'NESR',
  'NET',
  'NETC',
  'NETI',
  'NEU',
  'NEWP',
  'NEWR',
  'NEWT',
  'NEX',
  'NEXA',
  'NEXI',
  'NEXT',
  'NFBK',
  'NFE',
  'NFG',
  'NFGC',
  'NFLX',
  'NFNT',
  'NFYS',
  'NG',
  'NGC',
  'NGD',
  'NGG',
  'NGL',
  'NGM',
  'NGMS',
  'NGS',
  'NGVC',
  'NGVT',
  'NH',
  'NHC',
  'NHI',
  'NHIC',
  'NHTC',
  'NHWK',
  'NI',
  'NIC',
  'NICE',
  'NICK',
  'NILE',
  'NINE',
  'NIO',
  'NISN',
  'NIU',
  'NJR',
  'NKE',
  'NKLA',
  'NKSH',
  'NKTR',
  'NKTX',
  'NL',
  'NLIT',
  'NLOK',
  'NLS',
  'NLSN',
  'NLSP',
  'NLTX',
  'NLY',
  'NM',
  'NMFC',
  'NMG',
  'NMIH',
  'NMM',
  'NMMC',
  'NMR',
  'NMRD',
  'NMRK',
  'NMTC',
  'NMTR',
  'NN',
  'NNBR',
  'NNDM',
  'NNI',
  'NNN',
  'NNOX',
  'NNVC',
  'NOA',
  'NOAC',
  'NOAH',
  'NOC',
  'NODK',
  'NOG',
  'NOK',
  'NOMD',
  'NOTV',
  'NOV',
  'NOVA',
  'NOVN',
  'NOVT',
  'NOVV',
  'NOW',
  'NPAB',
  'NPCE',
  'NPK',
  'NPO',
  'NPTN',
  'NR',
  'NRAC',
  'NRBO',
  'NRC',
  'NRDS',
  'NRDY',
  'NREF',
  'NRG',
  'NRGV',
  'NRIM',
  'NRIX',
  'NRP',
  'NRSN',
  'NRT',
  'NRXP',
  'NRZ',
  'NS',
  'NSA',
  'NSC',
  'NSIT',
  'NSP',
  'NSPR',
  'NSR',
  'NSSC',
  'NSTB',
  'NSTC',
  'NSTD',
  'NSTG',
  'NSTS',
  'NSYS',
  'NTAP',
  'NTB',
  'NTCO',
  'NTCT',
  'NTES',
  'NTGR',
  'NTIC',
  'NTIP',
  'NTLA',
  'NTNX',
  'NTP',
  'NTR',
  'NTRA',
  'NTRB',
  'NTRS',
  'NTST',
  'NTUS',
  'NTWK',
  'NTZ',
  'NU',
  'NUBI',
  'NUE',
  'NURO',
  'NUS',
  'NUTX',
  'NUVA',
  'NUVB',
  'NUVL',
  'NUWE',
  'NUZE',
  'NVAC',
  'NVAX',
  'NVCN',
  'NVCR',
  'NVCT',
  'NVDA',
  'NVEC',
  'NVEE',
  'NVEI',
  'NVFY',
  'NVGS',
  'NVIV',
  'NVMI',
  'NVNO',
  'NVO',
  'NVOS',
  'NVR',
  'NVRO',
  'NVS',
  'NVSA',
  'NVST',
  'NVT',
  'NVTA',
  'NVTS',
  'NVVE',
  'NVX',
  'NWBI',
  'NWE',
  'NWFL',
  'NWG',
  'NWL',
  'NWLI',
  'NWN',
  'NWPX',
  'NWS',
  'NWSA',
  'NX',
  'NXE',
  'NXGL',
  'NXGN',
  'NXPI',
  'NXPL',
  'NXRT',
  'NXST',
  'NXTC',
  'NXTP',
  'NYC',
  'NYCB',
  'NYMT',
  'NYMX',
  'NYT',
  'NYXH',
  'O',
  'OB',
  'OBCI',
  'OBE',
  'OBLG',
  'OBNK',
  'OBSV',
  'OBT',
  'OC',
  'OCAX',
  'OCC',
  'OCCI',
  'OCFC',
  'OCFT',
  'OCG',
  'OCGN',
  'OCN',
  'OCSL',
  'OCUL',
  'OCUP',
  'OCX',
  'ODC',
  'ODFL',
  'ODP',
  'ODV',
  'OEC',
  'OEG',
  'OEPW',
  'OESX',
  'OFC',
  'OFED',
  'OFG',
  'OFIX',
  'OFLX',
  'OFS',
  'OG',
  'OGE',
  'OGEN',
  'OGI',
  'OGN',
  'OGS',
  'OHAA',
  'OHI',
  'OHPA',
  'OI',
  'OII',
  'OIIM',
  'OIS',
  'OKE',
  'OKTA',
  'OKYO',
  'OLB',
  'OLED',
  'OLIT',
  'OLK',
  'OLLI',
  'OLMA',
  'OLN',
  'OLO',
  'OLP',
  'OLPX',
  'OM',
  'OMAB',
  'OMC',
  'OMCL',
  'OMEG',
  'OMER',
  'OMEX',
  'OMF',
  'OMGA',
  'OMI',
  'OMIC',
  'OMQS',
  'ON',
  'ONB',
  'ONCR',
  'ONCS',
  'ONCT',
  'ONCY',
  'ONDS',
  'ONEM',
  'ONEW',
  'ONL',
  'ONON',
  'ONTF',
  'ONTO',
  'ONTX',
  'ONVO',
  'ONYX',
  'OOMA',
  'OP',
  'OPA',
  'OPAD',
  'OPBK',
  'OPCH',
  'OPEN',
  'OPFI',
  'OPGN',
  'OPHC',
  'OPI',
  'OPK',
  'OPNT',
  'OPOF',
  'OPRA',
  'OPRT',
  'OPRX',
  'OPT',
  'OPTN',
  'OPTT',
  'OPY',
  'OR',
  'ORA',
  'ORAN',
  'ORC',
  'ORCC',
  'ORCL',
  'ORGN',
  'ORGO',
  'ORGS',
  'ORI',
  'ORIA',
  'ORIC',
  'ORLA',
  'ORLY',
  'ORMP',
  'ORN',
  'ORRF',
  'ORTX',
  'OSBC',
  'OSCR',
  'OSG',
  'OSH',
  'OSI',
  'OSIS',
  'OSK',
  'OSPN',
  'OSS',
  'OST',
  'OSTK',
  'OSTR',
  'OSUR',
  'OSW',
  'OTEC',
  'OTEX',
  'OTIC',
  'OTIS',
  'OTLK',
  'OTLY',
  'OTMO',
  'OTRK',
  'OTTR',
  'OUST',
  'OUT',
  'OVBC',
  'OVID',
  'OVLY',
  'OVV',
  'OWL',
  'OWLT',
  'OXAC',
  'OXBR',
  'OXM',
  'OXSQ',
  'OXUS',
  'OXY',
  'OYST',
  'OZ',
  'OZK',
  'OZON',
  'PAA',
  'PAAS',
  'PAC',
  'PACB',
  'PACI',
  'PACK',
  'PACW',
  'PACX',
  'PAFO',
  'PAG',
  'PAGP',
  'PAGS',
  'PAHC',
  'PALI',
  'PALT',
  'PAM',
  'PANA',
  'PANL',
  'PANW',
  'PAQC',
  'PAR',
  'PARA',
  'PARA',
  'PARR',
  'PASG',
  'PATH',
  'PATI',
  'PATK',
  'PAVM',
  'PAX',
  'PAY',
  'PAYA',
  'PAYC',
  'PAYO',
  'PAYS',
  'PAYX',
  'PB',
  'PBA',
  'PBAX',
  'PBBK',
  'PBF',
  'PBFS',
  'PBFX',
  'PBH',
  'PBHC',
  'PBI',
  'PBLA',
  'PBPB',
  'PBR',
  'PBR.',
  'PBT',
  'PBTS',
  'PBYI',
  'PCAR',
  'PCB',
  'PCCT',
  'PCG',
  'PCH',
  'PCOR',
  'PCPC',
  'PCRX',
  'PCSA',
  'PCSB',
  'PCT',
  'PCTI',
  'PCTY',
  'PCVX',
  'PCX',
  'PCYG',
  'PCYO',
  'PD',
  'PDCE',
  'PDCO',
  'PDD',
  'PDEX',
  'PDFS',
  'PDLB',
  'PDM',
  'PDOT',
  'PDS',
  'PDSB',
  'PEAK',
  'PEAR',
  'PEB',
  'PEBK',
  'PEBO',
  'PECO',
  'PED',
  'PEG',
  'PEGA',
  'PEGR',
  'PEGY',
  'PEI',
  'PEN',
  'PENN',
  'PEP',
  'PEPG',
  'PEPL',
  'PERI',
  'PESI',
  'PETQ',
  'PETS',
  'PETV',
  'PETZ',
  'PEV',
  'PFBC',
  'PFC',
  'PFDR',
  'PFE',
  'PFG',
  'PFGC',
  'PFHC',
  'PFHD',
  'PFIE',
  'PFIN',
  'PFIS',
  'PFLT',
  'PFMT',
  'PFS',
  'PFSI',
  'PFSW',
  'PFTA',
  'PFX',
  'PG',
  'PGC',
  'PGEN',
  'PGNY',
  'PGR',
  'PGRE',
  'PGRU',
  'PGRW',
  'PGSS',
  'PGTI',
  'PGY',
  'PH',
  'PHAR',
  'PHAS',
  'PHAT',
  'PHCF',
  'PHG',
  'PHGE',
  'PHI',
  'PHIC',
  'PHIO',
  'PHM',
  'PHR',
  'PHUN',
  'PHVS',
  'PHX',
  'PHYT',
  'PI',
  'PIAI',
  'PICC',
  'PII',
  'PIII',
  'PIK',
  'PINC',
  'PINE',
  'PING',
  'PINS',
  'PIPP',
  'PIPR',
  'PIRS',
  'PIXY',
  'PJT',
  'PK',
  'PKBK',
  'PKE',
  'PKG',
  'PKI',
  'PKOH',
  'PKX',
  'PL',
  'PLAB',
  'PLAG',
  'PLAO',
  'PLAY',
  'PLBC',
  'PLBY',
  'PLCE',
  'PLD',
  'PLG',
  'PLL',
  'PLM',
  'PLMI',
  'PLMR',
  'PLNT',
  'PLOW',
  'PLPC',
  'PLRX',
  'PLSE',
  'PLTK',
  'PLTR',
  'PLUG',
  'PLUS',
  'PLX',
  'PLXP',
  'PLXS',
  'PLYA',
  'PLYM',
  'PM',
  'PMCB',
  'PMD',
  'PME',
  'PMGM',
  'PMN',
  'PMT',
  'PMTS',
  'PMVC',
  'PMVP',
  'PNAC',
  'PNBK',
  'PNC',
  'PNFP',
  'PNM',
  'PNNT',
  'PNR',
  'PNRG',
  'PNT',
  'PNTG',
  'PNTM',
  'PNW',
  'POAI',
  'PODD',
  'POET',
  'POLA',
  'POLY',
  'POND',
  'PONO',
  'POOL',
  'POR',
  'PORT',
  'POSH',
  'POST',
  'POW',
  'POWI',
  'POWL',
  'POWW',
  'PPBI',
  'PPBT',
  'PPC',
  'PPG',
  'PPHP',
  'PPIH',
  'PPL',
  'PPSI',
  'PPTA',
  'PPYA',
  'PRA',
  'PRAA',
  'PRAX',
  'PRBM',
  'PRCH',
  'PRCT',
  'PRDO',
  'PRDS',
  'PRE',
  'PRFT',
  'PRFX',
  'PRG',
  'PRGO',
  'PRGS',
  'PRI',
  'PRIM',
  'PRK',
  'PRLB',
  'PRLD',
  'PRLH',
  'PRM',
  'PRO',
  'PROC',
  'PROF',
  'PROV',
  'PRPB',
  'PRPC',
  'PRPH',
  'PRPL',
  'PRPO',
  'PRQR',
  'PRSO',
  'PRSR',
  'PRT',
  'PRTA',
  'PRTC',
  'PRTG',
  'PRTH',
  'PRTK',
  'PRTS',
  'PRTY',
  'PRU',
  'PRVA',
  'PRVB',
  'PSA',
  'PSAG',
  'PSB',
  'PSEC',
  'PSFE',
  'PSHG',
  'PSMT',
  'PSN',
  'PSNL',
  'PSNY',
  'PSO',
  'PSPC',
  'PSTG',
  'PSTH',
  'PSTI',
  'PSTL',
  'PSTV',
  'PSTX',
  'PSX',
  'PT',
  'PTC',
  'PTCT',
  'PTE',
  'PTEN',
  'PTGX',
  'PTIC',
  'PTIX',
  'PTLO',
  'PTMN',
  'PTN',
  'PTNR',
  'PTOC',
  'PTON',
  'PTPI',
  'PTR',
  'PTRA',
  'PTRS',
  'PTSI',
  'PTVE',
  'PUBM',
  'PUCK',
  'PUK',
  'PULM',
  'PUMP',
  'PUYI',
  'PV',
  'PVBC',
  'PVH',
  'PVL',
  'PW',
  'PWFL',
  'PWOD',
  'PWP',
  'PWR',
  'PWSC',
  'PWUP',
  'PX',
  'PXD',
  'PXLW',
  'PXS',
  'PYCR',
  'PYPD',
  'PYPL',
  'PYR',
  'PYXS',
  'PZG',
  'PZN',
  'PZZA',
  'QCOM',
  'QCRH',
  'QD',
  'QDEL',
  'QFIN',
  'QFTA',
  'QGEN',
  'QH',
  'QIPT',
  'QIWI',
  'QK',
  'QLGN',
  'QLI',
  'QLYS',
  'QMCO',
  'QNGY',
  'QNRX',
  'QNST',
  'QRHC',
  'QRTE',
  'QRTE',
  'QRVO',
  'QS',
  'QSI',
  'QSR',
  'QTEK',
  'QTNT',
  'QTRX',
  'QTT',
  'QTWO',
  'QUAD',
  'QUBT',
  'QUIK',
  'QUMU',
  'QUOT',
  'QURE',
  'R',
  'RAAS',
  'RACB',
  'RACE',
  'RACY',
  'RAD',
  'RADA',
  'RADI',
  'RAIL',
  'RAIN',
  'RAM',
  'RAMP',
  'RAND',
  'RANI',
  'RAPT',
  'RARE',
  'RAVE',
  'RBA',
  'RBAC',
  'RBB',
  'RBBN',
  'RBCA',
  'RBCN',
  'RBKB',
  'RBLX',
  'RBOT',
  'RC',
  'RCAC',
  'RCAT',
  'RCEL',
  'RCFA',
  'RCHG',
  'RCI',
  'RCII',
  'RCKT',
  'RCKY',
  'RCL',
  'RCLF',
  'RCM',
  'RCMT',
  'RCON',
  'RCOR',
  'RCRT',
  'RCUS',
  'RDBX',
  'RDCM',
  'RDFN',
  'RDHL',
  'RDI',
  'RDIB',
  'RDN',
  'RDNT',
  'RDUS',
  'RDVT',
  'RDW',
  'RDWR',
  'RDY',
  'RE',
  'REAL',
  'REAX',
  'REE',
  'REED',
  'REFI',
  'REFR',
  'REG',
  'REGN',
  'REI',
  'REKR',
  'RELI',
  'RELL',
  'RELX',
  'RELY',
  'RENE',
  'RENN',
  'RENO',
  'RENT',
  'REPL',
  'REPX',
  'RERE',
  'RES',
  'RETA',
  'RETO',
  'REV',
  'REVB',
  'REVE',
  'REVG',
  'REVH',
  'REX',
  'REXR',
  'REYN',
  'REZI',
  'RF',
  'RFAC',
  'RFIL',
  'RFL',
  'RFP',
  'RGA',
  'RGC',
  'RGCO',
  'RGEN',
  'RGF',
  'RGLD',
  'RGLS',
  'RGNX',
  'RGP',
  'RGR',
  'RGS',
  'RGTI',
  'RH',
  'RHE',
  'RHI',
  'RHP',
  'RIBT',
  'RICK',
  'RIDE',
  'RIG',
  'RIGL',
  'RILY',
  'RIO',
  'RIOT',
  'RIVN',
  'RJAC',
  'RJF',
  'RKDA',
  'RKLB',
  'RKLY',
  'RKT',
  'RKTA',
  'RL',
  'RLAY',
  'RLGT',
  'RLI',
  'RLJ',
  'RLMD',
  'RLX',
  'RLYB',
  'RM',
  'RMAX',
  'RMBI',
  'RMBL',
  'RMBS',
  'RMCF',
  'RMD',
  'RMED',
  'RMGC',
  'RMNI',
  'RMO',
  'RMR',
  'RMTI',
  'RNA',
  'RNAZ',
  'RNDB',
  'RNER',
  'RNG',
  'RNGR',
  'RNLX',
  'RNR',
  'RNST',
  'RNW',
  'RNWK',
  'RNXT',
  'ROAD',
  'ROC',
  'ROCC',
  'ROCG',
  'ROCK',
  'ROCL',
  'ROG',
  'ROIC',
  'ROIV',
  'ROK',
  'ROKU',
  'ROL',
  'ROLL',
  'RONI',
  'ROOT',
  'ROP',
  'ROSE',
  'ROSS',
  'ROST',
  'ROVR',
  'RPAY',
  'RPD',
  'RPHM',
  'RPID',
  'RPM',
  'RPRX',
  'RPT',
  'RPTX',
  'RRAC',
  'RRBI',
  'RRC',
  'RRGB',
  'RRR',
  'RRX',
  'RS',
  'RSG',
  'RSI',
  'RSKD',
  'RSLS',
  'RSSS',
  'RSVR',
  'RTL',
  'RTLR',
  'RTX',
  'RUBY',
  'RUN',
  'RUSH',
  'RUSH',
  'RUTH',
  'RVAC',
  'RVLP',
  'RVLV',
  'RVMD',
  'RVNC',
  'RVP',
  'RVPH',
  'RVSB',
  'RVSN',
  'RWAY',
  'RWLK',
  'RWOD',
  'RWT',
  'RXDX',
  'RXRA',
  'RXRX',
  'RXST',
  'RXT',
  'RY',
  'RYAA',
  'RYAM',
  'RYAN',
  'RYI',
  'RYN',
  'RYTM',
  'RZLT',
  'S',
  'SA',
  'SABR',
  'SABS',
  'SACH',
  'SAFE',
  'SAFM',
  'SAFT',
  'SAGA',
  'SAGE',
  'SAH',
  'SAI',
  'SAIA',
  'SAIC',
  'SAIL',
  'SAL',
  'SALM',
  'SAM',
  'SAMA',
  'SAMG',
  'SAN',
  'SANA',
  'SANB',
  'SAND',
  'SANG',
  'SANM',
  'SANW',
  'SAP',
  'SAR',
  'SASI',
  'SASR',
  'SATL',
  'SATS',
  'SAVA',
  'SAVE',
  'SB',
  'SBAC',
  'SBCF',
  'SBET',
  'SBEV',
  'SBFG',
  'SBGI',
  'SBH',
  'SBIG',
  'SBII',
  'SBLK',
  'SBNY',
  'SBOW',
  'SBR',
  'SBRA',
  'SBS',
  'SBSI',
  'SBSW',
  'SBT',
  'SBTX',
  'SBUX',
  'SCAQ',
  'SCCO',
  'SCHL',
  'SCHN',
  'SCHW',
  'SCI',
  'SCKT',
  'SCL',
  'SCLE',
  'SCM',
  'SCMA',
  'SCOA',
  'SCOB',
  'SCOR',
  'SCPH',
  'SCPL',
  'SCPS',
  'SCRM',
  'SCS',
  'SCSC',
  'SCTL',
  'SCU',
  'SCUA',
  'SCVL',
  'SCVX',
  'SCWO',
  'SCWX',
  'SCX',
  'SCYX',
  'SD',
  'SDAC',
  'SDC',
  'SDGR',
  'SDH',
  'SDIG',
  'SDPI',
  'SE',
  'SEAC',
  'SEAS',
  'SEAT',
  'SEB',
  'SECO',
  'SEDA',
  'SEDG',
  'SEE',
  'SEED',
  'SEEL',
  'SEER',
  'SEIC',
  'SELB',
  'SELF',
  'SEM',
  'SEMR',
  'SENE',
  'SENE',
  'SENS',
  'SERA',
  'SES',
  'SESN',
  'SEV',
  'SEVN',
  'SF',
  'SFBC',
  'SFBS',
  'SFE',
  'SFET',
  'SFIX',
  'SFL',
  'SFM',
  'SFNC',
  'SFST',
  'SFT',
  'SG',
  'SGA',
  'SGBX',
  'SGC',
  'SGEN',
  'SGFY',
  'SGH',
  'SGHC',
  'SGHL',
  'SGHT',
  'SGII',
  'SGLY',
  'SGMA',
  'SGML',
  'SGMO',
  'SGRP',
  'SGRY',
  'SGTX',
  'SGU',
  'SHAC',
  'SHAK',
  'SHAP',
  'SHBI',
  'SHC',
  'SHCA',
  'SHCR',
  'SHEL',
  'SHEN',
  'SHG',
  'SHI',
  'SHIP',
  'SHLS',
  'SHLX',
  'SHO',
  'SHOO',
  'SHOP',
  'SHPW',
  'SHQA',
  'SHUA',
  'SHW',
  'SHYF',
  'SI',
  'SIBN',
  'SID',
  'SIDU',
  'SIEB',
  'SIEN',
  'SIER',
  'SIF',
  'SIFY',
  'SIG',
  'SIGA',
  'SIGI',
  'SII',
  'SILC',
  'SILK',
  'SILV',
  'SIM',
  'SIMO',
  'SINT',
  'SIOX',
  'SIRE',
  'SIRI',
  'SISI',
  'SITC',
  'SITE',
  'SITM',
  'SIVB',
  'SIX',
  'SJ',
  'SJI',
  'SJM',
  'SJR',
  'SJT',
  'SJW',
  'SKE',
  'SKGR',
  'SKIL',
  'SKIN',
  'SKLZ',
  'SKM',
  'SKT',
  'SKX',
  'SKY',
  'SKYA',
  'SKYH',
  'SKYT',
  'SKYW',
  'SKYX',
  'SLAB',
  'SLAC',
  'SLAM',
  'SLB',
  'SLCA',
  'SLCR',
  'SLDB',
  'SLDP',
  'SLF',
  'SLG',
  'SLGC',
  'SLGG',
  'SLGL',
  'SLGN',
  'SLHG',
  'SLI',
  'SLM',
  'SLN',
  'SLNG',
  'SLNH',
  'SLNO',
  'SLP',
  'SLQT',
  'SLRC',
  'SLRX',
  'SLS',
  'SLVM',
  'SLVR',
  'SM',
  'SMAP',
  'SMAR',
  'SMBC',
  'SMBK',
  'SMCI',
  'SMED',
  'SMFG',
  'SMFL',
  'SMFR',
  'SMG',
  'SMHI',
  'SMID',
  'SMIH',
  'SMIT',
  'SMLP',
  'SMLR',
  'SMMF',
  'SMMT',
  'SMP',
  'SMPL',
  'SMR',
  'SMRT',
  'SMSI',
  'SMTC',
  'SMTI',
  'SMTS',
  'SMWB',
  'SNA',
  'SNAP',
  'SNAX',
  'SNBR',
  'SNCE',
  'SNCR',
  'SNCY',
  'SND',
  'SNDA',
  'SNDL',
  'SNDR',
  'SNDX',
  'SNES',
  'SNEX',
  'SNFC',
  'SNGX',
  'SNMP',
  'SNN',
  'SNOA',
  'SNOW',
  'SNP',
  'SNPO',
  'SNPS',
  'SNPX',
  'SNRH',
  'SNSE',
  'SNT',
  'SNTG',
  'SNTI',
  'SNV',
  'SNX',
  'SNY',
  'SO',
  'SOFI',
  'SOHO',
  'SOHU',
  'SOI',
  'SOL',
  'SOLO',
  'SON',
  'SOND',
  'SONM',
  'SONN',
  'SONO',
  'SONX',
  'SONY',
  'SOPA',
  'SOPH',
  'SOS',
  'SOTK',
  'SOUN',
  'SOVO',
  'SP',
  'SPB',
  'SPCB',
  'SPCE',
  'SPCM',
  'SPFI',
  'SPG',
  'SPGI',
  'SPGS',
  'SPH',
  'SPI',
  'SPIR',
  'SPK',
  'SPKB',
  'SPLK',
  'SPLP',
  'SPNE',
  'SPNS',
  'SPNT',
  'SPOK',
  'SPOT',
  'SPPI',
  'SPR',
  'SPRB',
  'SPRC',
  'SPRO',
  'SPSC',
  'SPT',
  'SPTK',
  'SPTN',
  'SPWH',
  'SPWR',
  'SPXC',
  'SQ',
  'SQFT',
  'SQL',
  'SQM',
  'SQNS',
  'SQSP',
  'SQZ',
  'SR',
  'SRAD',
  'SRAX',
  'SRC',
  'SRCE',
  'SRCL',
  'SRDX',
  'SRE',
  'SREV',
  'SRG',
  'SRGA',
  'SRI',
  'SRL',
  'SRLP',
  'SRNE',
  'SRPT',
  'SRRK',
  'SRSA',
  'SRT',
  'SRTS',
  'SRZN',
  'SSAA',
  'SSB',
  'SSBI',
  'SSBK',
  'SSD',
  'SSIC',
  'SSKN',
  'SSL',
  'SSNC',
  'SSNT',
  'SSP',
  'SSRM',
  'SSSS',
  'SST',
  'SSTI',
  'SSTK',
  'SSU',
  'SSY',
  'SSYS',
  'ST',
  'STAA',
  'STAB',
  'STAF',
  'STAG',
  'STAR',
  'STBA',
  'STC',
  'STCN',
  'STE',
  'STEM',
  'STEP',
  'STER',
  'STET',
  'STG',
  'STGW',
  'STIM',
  'STKL',
  'STKS',
  'STLA',
  'STLD',
  'STM',
  'STN',
  'STNE',
  'STNG',
  'STOK',
  'STON',
  'STOR',
  'STR',
  'STRA',
  'STRC',
  'STRE',
  'STRL',
  'STRM',
  'STRN',
  'STRO',
  'STRR',
  'STRS',
  'STRT',
  'STRY',
  'STSA',
  'STSS',
  'STT',
  'STTK',
  'STVN',
  'STWD',
  'STX',
  'STXS',
  'STZ',
  'SU',
  'SUAC',
  'SUI',
  'SUM',
  'SUMO',
  'SUN',
  'SUNL',
  'SUNW',
  'SUP',
  'SUPN',
  'SUPV',
  'SURF',
  'SURG',
  'SUZ',
  'SVC',
  'SVFA',
  'SVFB',
  'SVFD',
  'SVM',
  'SVNA',
  'SVRA',
  'SVRE',
  'SVT',
  'SVVC',
  'SWAG',
  'SWAV',
  'SWBI',
  'SWCH',
  'SWET',
  'SWI',
  'SWIM',
  'SWIR',
  'SWK',
  'SWKH',
  'SWKS',
  'SWN',
  'SWSS',
  'SWTX',
  'SWVL',
  'SWX',
  'SXC',
  'SXI',
  'SXT',
  'SXTC',
  'SY',
  'SYBT',
  'SYBX',
  'SYF',
  'SYK',
  'SYM',
  'SYN',
  'SYNA',
  'SYNH',
  'SYNL',
  'SYPR',
  'SYRS',
  'SYTA',
  'SYY',
  'SZZL',
  'T',
  'TA',
  'TAC',
  'TACT',
  'TAIT',
  'TAK',
  'TAL',
  'TALK',
  'TALO',
  'TALS',
  'TANH',
  'TAOP',
  'TAP',
  'TARA',
  'TARO',
  'TARS',
  'TASK',
  'TAST',
  'TATT',
  'TAYD',
  'TBBK',
  'TBCP',
  'TBI',
  'TBK',
  'TBLA',
  'TBLD',
  'TBLT',
  'TBNK',
  'TBPH',
  'TBSA',
  'TC',
  'TCBC',
  'TCBI',
  'TCBK',
  'TCBP',
  'TCBS',
  'TCBX',
  'TCDA',
  'TCFC',
  'TCI',
  'TCMD',
  'TCN',
  'TCOA',
  'TCOM',
  'TCON',
  'TCPC',
  'TCRR',
  'TCRT',
  'TCRX',
  'TCS',
  'TCVA',
  'TCX',
  'TD',
  'TDC',
  'TDCX',
  'TDG',
  'TDOC',
  'TDS',
  'TDUP',
  'TDW',
  'TDY',
  'TEAM',
  'TECH',
  'TECK',
  'TEDU',
  'TEF',
  'TEKK',
  'TEL',
  'TELA',
  'TELL',
  'TEN',
  'TENB',
  'TENX',
  'TEO',
  'TER',
  'TERN',
  'TESS',
  'TETC',
  'TETE',
  'TEVA',
  'TEX',
  'TFC',
  'TFFP',
  'TFII',
  'TFSL',
  'TFX',
  'TG',
  'TGA',
  'TGAA',
  'TGB',
  'TGH',
  'TGI',
  'TGLS',
  'TGNA',
  'TGR',
  'TGS',
  'TGT',
  'TGTX',
  'TGVC',
  'TH',
  'THAC',
  'THC',
  'THCA',
  'THCP',
  'THFF',
  'THG',
  'THM',
  'THMO',
  'THO',
  'THR',
  'THRM',
  'THRN',
  'THRX',
  'THRY',
  'THS',
  'THTX',
  'TIG',
  'TIGO',
  'TIGR',
  'TIL',
  'TILE',
  'TIMB',
  'TINV',
  'TIOA',
  'TIPT',
  'TIRX',
  'TISI',
  'TITN',
  'TIVC',
  'TIXT',
  'TJX',
  'TK',
  'TKAT',
  'TKC',
  'TKLF',
  'TKNO',
  'TKR',
  'TLGA',
  'TLGY',
  'TLIS',
  'TLK',
  'TLRY',
  'TLS',
  'TLSA',
  'TLYS',
  'TM',
  'TMAC',
  'TMBR',
  'TMC',
  'TMCI',
  'TMDI',
  'TMDX',
  'TME',
  'TMHC',
  'TMKR',
  'TMO',
  'TMP',
  'TMPM',
  'TMQ',
  'TMST',
  'TMUS',
  'TMX',
  'TNC',
  'TNDM',
  'TNET',
  'TNGX',
  'TNK',
  'TNL',
  'TNON',
  'TNP',
  'TNXP',
  'TNYA',
  'TOAC',
  'TOI',
  'TOL',
  'TOMZ',
  'TOP',
  'TOPS',
  'TOST',
  'TOUR',
  'TOWN',
  'TPB',
  'TPBA',
  'TPC',
  'TPG',
  'TPGY',
  'TPH',
  'TPHS',
  'TPIC',
  'TPL',
  'TPR',
  'TPST',
  'TPTX',
  'TPVG',
  'TPX',
  'TR',
  'TRAQ',
  'TRC',
  'TRCA',
  'TRDA',
  'TREE',
  'TREX',
  'TRGP',
  'TRHC',
  'TRI',
  'TRIB',
  'TRIN',
  'TRIP',
  'TRIS',
  'TRKA',
  'TRMB',
  'TRMD',
  'TRMK',
  'TRMR',
  'TRN',
  'TRNO',
  'TRNS',
  'TRON',
  'TROO',
  'TROW',
  'TROX',
  'TRP',
  'TRQ',
  'TRS',
  'TRST',
  'TRT',
  'TRTL',
  'TRTN',
  'TRTX',
  'TRU',
  'TRUE',
  'TRUP',
  'TRV',
  'TRVG',
  'TRVI',
  'TRVN',
  'TRX',
  'TS',
  'TSAT',
  'TSBK',
  'TSCO',
  'TSE',
  'TSEM',
  'TSHA',
  'TSIB',
  'TSLA',
  'TSLX',
  'TSM',
  'TSN',
  'TSP',
  'TSPQ',
  'TSQ',
  'TSRI',
  'TSVT',
  'TT',
  'TTC',
  'TTCF',
  'TTD',
  'TTE',
  'TTEC',
  'TTEK',
  'TTGT',
  'TTI',
  'TTM',
  'TTMI',
  'TTNP',
  'TTOO',
  'TTSH',
  'TTWO',
  'TU',
  'TUEM',
  'TUFN',
  'TUP',
  'TURN',
  'TUSK',
  'TUYA',
  'TV',
  'TVTX',
  'TVTY',
  'TW',
  'TWCB',
  'TWI',
  'TWIN',
  'TWKS',
  'TWLO',
  'TWLV',
  'TWND',
  'TWNI',
  'TWNK',
  'TWO',
  'TWOA',
  'TWOU',
  'TWST',
  'TWTR',
  'TX',
  'TXG',
  'TXMD',
  'TXN',
  'TXRH',
  'TXT',
  'TYDE',
  'TYG',
  'TYL',
  'TYME',
  'TYRA',
  'TZOO',
  'TZPS',
  'U',
  'UA',
  'UAA',
  'UAL',
  'UAMY',
  'UAN',
  'UAVS',
  'UBA',
  'UBCP',
  'UBER',
  'UBFO',
  'UBOH',
  'UBP',
  'UBS',
  'UBSI',
  'UBX',
  'UCBI',
  'UCL',
  'UCTT',
  'UDMY',
  'UDR',
  'UE',
  'UEC',
  'UEIC',
  'UFAB',
  'UFCS',
  'UFI',
  'UFPI',
  'UFPT',
  'UG',
  'UGI',
  'UGP',
  'UGRO',
  'UHAL',
  'UHS',
  'UHT',
  'UI',
  'UIHC',
  'UIS',
  'UK',
  'UL',
  'ULBI',
  'ULCC',
  'ULH',
  'ULTA',
  'UMBF',
  'UMC',
  'UMH',
  'UMPQ',
  'UNAM',
  'UNB',
  'UNCY',
  'UNF',
  'UNFI',
  'UNH',
  'UNIT',
  'UNM',
  'UNP',
  'UNTY',
  'UNVR',
  'UONE',
  'UONE',
  'UP',
  'UPC',
  'UPH',
  'UPLD',
  'UPS',
  'UPST',
  'UPTD',
  'UPWK',
  'URBN',
  'URG',
  'URGN',
  'URI',
  'UROY',
  'USAC',
  'USAK',
  'USAP',
  'USAS',
  'USAU',
  'USB',
  'USCB',
  'USCT',
  'USDP',
  'USEA',
  'USEG',
  'USER',
  'USFD',
  'USIO',
  'USLM',
  'USM',
  'USNA',
  'USPH',
  'USWS',
  'USX',
  'UTAA',
  'UTHR',
  'UTI',
  'UTL',
  'UTMD',
  'UTME',
  'UTRS',
  'UTSI',
  'UTZ',
  'UUU',
  'UUUU',
  'UVE',
  'UVSP',
  'UVV',
  'UWMC',
  'UXIN',
  'V',
  'VABK',
  'VAC',
  'VACC',
  'VAL',
  'VALE',
  'VALN',
  'VALU',
  'VAPO',
  'VAQC',
  'VATE',
  'VAXX',
  'VBFC',
  'VBIV',
  'VBLT',
  'VBNK',
  'VBOC',
  'VBTX',
  'VC',
  'VCEL',
  'VCKA',
  'VCNX',
  'VCSA',
  'VCTR',
  'VCXA',
  'VCXB',
  'VCYT',
  'VECO',
  'VECT',
  'VEDU',
  'VEEE',
  'VEEV',
  'VEL',
  'VELO',
  'VENA',
  'VEON',
  'VERA',
  'VERB',
  'VERI',
  'VERO',
  'VERU',
  'VERV',
  'VERX',
  'VERY',
  'VET',
  'VEV',
  'VFC',
  'VFF',
  'VG',
  'VGFC',
  'VGR',
  'VGZ',
  'VHAQ',
  'VHC',
  'VHI',
  'VHNA',
  'VIA',
  'VIAO',
  'VIAV',
  'VICI',
  'VICR',
  'VIEW',
  'VIGL',
  'VII',
  'VINC',
  'VINE',
  'VINO',
  'VINP',
  'VIOT',
  'VIPS',
  'VIR',
  'VIRC',
  'VIRI',
  'VIRT',
  'VIRX',
  'VISL',
  'VIST',
  'VITL',
  'VIV',
  'VIVE',
  'VIVO',
  'VJET',
  'VKTX',
  'VLAT',
  'VLCN',
  'VLD',
  'VLDR',
  'VLGE',
  'VLN',
  'VLNS',
  'VLO',
  'VLON',
  'VLRS',
  'VLTA',
  'VLY',
  'VMAR',
  'VMC',
  'VMCA',
  'VMD',
  'VMEO',
  'VMGA',
  'VMI',
  'VMW',
  'VNCE',
  'VNDA',
  'VNET',
  'VNO',
  'VNOM',
  'VNRX',
  'VNT',
  'VNTR',
  'VOC',
  'VOD',
  'VOR',
  'VORB',
  'VOXX',
  'VOYA',
  'VPCB',
  'VPG',
  'VQS',
  'VRA',
  'VRAR',
  'VRAY',
  'VRCA',
  'VRDN',
  'VRE',
  'VREX',
  'VRM',
  'VRME',
  'VRNA',
  'VRNS',
  'VRNT',
  'VRPX',
  'VRRM',
  'VRSK',
  'VRSN',
  'VRT',
  'VRTS',
  'VRTV',
  'VRTX',
  'VS',
  'VSAC',
  'VSAT',
  'VSCO',
  'VSEC',
  'VSH',
  'VST',
  'VSTA',
  'VSTM',
  'VSTO',
  'VTAQ',
  'VTEX',
  'VTGN',
  'VTIQ',
  'VTNR',
  'VTOL',
  'VTR',
  'VTRS',
  'VTRU',
  'VTSI',
  'VTVT',
  'VTYX',
  'VUZI',
  'VVI',
  'VVNT',
  'VVOS',
  'VVPR',
  'VVV',
  'VVX',
  'VWE',
  'VXRT',
  'VYGG',
  'VYGR',
  'VYNE',
  'VYNT',
  'VZ',
  'VZIO',
  'VZLA',
  'W',
  'WAB',
  'WABC',
  'WAFD',
  'WAFU',
  'WAL',
  'WALD',
  'WARR',
  'WASH',
  'WAT',
  'WATT',
  'WAVC',
  'WAVD',
  'WAVE',
  'WAVS',
  'WB',
  'WBA',
  'WBD',
  'WBEV',
  'WBS',
  'WBT',
  'WBX',
  'WCC',
  'WCN',
  'WD',
  'WDAY',
  'WDC',
  'WDFC',
  'WDH',
  'WDS',
  'WE',
  'WEAV',
  'WEBR',
  'WEC',
  'WEJO',
  'WEL',
  'WELL',
  'WEN',
  'WERN',
  'WES',
  'WETF',
  'WEX',
  'WEYS',
  'WF',
  'WFC',
  'WFCF',
  'WFG',
  'WFRD',
  'WGO',
  'WH',
  'WHD',
  'WHF',
  'WHG',
  'WHLM',
  'WHLR',
  'WHR',
  'WILC',
  'WIMI',
  'WINA',
  'WING',
  'WINT',
  'WINV',
  'WIRE',
  'WISA',
  'WISH',
  'WIT',
  'WIX',
  'WK',
  'WKEY',
  'WKHS',
  'WKME',
  'WKSP',
  'WLDN',
  'WLFC',
  'WLK',
  'WLKP',
  'WLMS',
  'WLY',
  'WLYB',
  'WM',
  'WMB',
  'WMC',
  'WMG',
  'WMK',
  'WMPN',
  'WMS',
  'WMT',
  'WNC',
  'WNEB',
  'WNNR',
  'WNS',
  'WNW',
  'WOLF',
  'WOOF',
  'WOR',
  'WORX',
  'WOW',
  'WPC',
  'WPCA',
  'WPCB',
  'WPM',
  'WPP',
  'WPRT',
  'WQGA',
  'WRAC',
  'WRAP',
  'WRB',
  'WRBY',
  'WRE',
  'WRK',
  'WRLD',
  'WRN',
  'WSBC',
  'WSBF',
  'WSC',
  'WSFS',
  'WSM',
  'WSO',
  'WSR',
  'WST',
  'WSTG',
  'WTBA',
  'WTER',
  'WTFC',
  'WTI',
  'WTM',
  'WTMA',
  'WTRG',
  'WTRH',
  'WTS',
  'WTT',
  'WTTR',
  'WTW',
  'WU',
  'WULF',
  'WVE',
  'WVVI',
  'WW',
  'WWAC',
  'WWD',
  'WWE',
  'WWR',
  'WWW',
  'WY',
  'WYNN',
  'WYY',
  'X',
  'XAIR',
  'XBIO',
  'XBIT',
  'XCUR',
  'XEL',
  'XELA',
  'XELB',
  'XENE',
  'XERS',
  'XFIN',
  'XFOR',
  'XGN',
  'XHR',
  'XIN',
  'XL',
  'XLO',
  'XM',
  'XMTR',
  'XNCR',
  'XNET',
  'XOM',
  'XOMA',
  'XOS',
  'XP',
  'XPAX',
  'XPDB',
  'XPEL',
  'XPER',
  'XPEV',
  'XPL',
  'XPO',
  'XPOA',
  'XPOF',
  'XPON',
  'XPRO',
  'XRAY',
  'XRTX',
  'XRX',
  'XSPA',
  'XTLB',
  'XTNT',
  'XXII',
  'XYF',
  'XYL',
  'Y',
  'YALA',
  'YCBD',
  'YELL',
  'YELP',
  'YETI',
  'YEXT',
  'YGMZ',
  'YI',
  'YJ',
  'YMAB',
  'YMM',
  'YMTX',
  'YNDX',
  'YORW',
  'YOTA',
  'YOU',
  'YPF',
  'YQ',
  'YRD',
  'YSG',
  'YTEN',
  'YTPG',
  'YTRA',
  'YUM',
  'YUMC',
  'YVR',
  'YY',
  'Z',
  'ZBH',
  'ZBRA',
  'ZCMD',
  'ZD',
  'ZDGE',
  'ZEAL',
  'ZEN',
  'ZENV',
  'ZEPP',
  'ZEST',
  'ZETA',
  'ZEUS',
  'ZEV',
  'ZG',
  'ZGN',
  'ZH',
  'ZI',
  'ZIM',
  'ZIMV',
  'ZING',
  'ZION',
  'ZIP',
  'ZIVO',
  'ZKIN',
  'ZLAB',
  'ZM',
  'ZNH',
  'ZNTL',
  'ZOM',
  'ZS',
  'ZSAN',
  'ZT',
  'ZTEK',
  'ZTO',
  'ZTS',
  'ZUMZ',
  'ZUO',
  'ZVIA',
  'ZVO',
  'ZWRK',
  'ZWS',
  'ZY',
  'ZYME',
  'ZYNE',
  'ZYXI',
];
